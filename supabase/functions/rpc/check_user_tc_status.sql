...
BEGIN
    ...
    -- Edge Case Fix: If no active T&C exists, user does NOT need an update
    IF v_latest_id IS NULL THEN
        RETURN QUERY
        SELECT false, NULL::uuid, NULL::jsonb, NULL::int;
        RETURN;
    END IF;
    ...
    -- 3. Return results (Explicitly maps variables to output columns)
    RETURN QUERY
    SELECT
        (COALESCE(v_user_agreed_version, 0) < v_latest_version) AS needs_update,
        v_latest_id AS latest_tc_id,
        v_latest_content AS latest_tc_content,
