...
export function ComponentSelectionView({
  projectUuid,
  bleResults,
  onBack,
  onCheckout,
}: ComponentSelectionViewProps) {
  ...
  const { items, categories, fetchItems, fetchCategories } =
    useInventoryStore();
  ...
  const [selectedSlotCategory, setSelectedSlotCategory] = useState<
    string | null
  >(null);
  ...
  const handleGenerateRecommendations = async () => {
    if (!bleResults) {
      toast.error(
        t(
          "components.no_ble_results",
          "No calculation results found. Please run calculation first.",
        ),
      );
      return;
    }
    ...
    try {
      ...
      if (result.length !== 0) {
        toast.success(
          t(
            "components.recommendations_generated",
  ...
  const getCategoryUuidsByAliases = (aliases: string[]) => {
    ...
    return categories
      .filter(
        (c) =>
          aliasSet.size > 0 &&
          Array.from(aliasSet).some((a) => normalizeName(c.name).includes(a)),
      )
      .map((c) => c.uuid);
  };
  ...
  const categoryUuidsBySlot = useMemo(
    () => ({
      ...
      battery: getCategoryUuidsByAliases([
        "battery",
        "battery bank",
        "batteries",
        "bank",
      ]),
      panel: getCategoryUuidsByAliases([
        "panel",
        "solar panel",
        "solar panels",
        "pv",
  ...
  const handleSelectItem = async (item: InventoryItem) => {
    try {
      ...
      const slotCategoryUuids = slotKey
        ? new Set(categoryUuidsBySlot[slotKey])
        : null;
      ...
      toast.success(
        t("components.item_added", "Item added to project components."),
      );
    } catch (e: any) {
  ...
  const findComponentByCategoryKey = (
    key: keyof typeof categoryUuidsBySlot,
  ) => {
    const allowed = new Set(categoryUuidsBySlot[key]);
    if (allowed.size === 0) {
      const needle = key.toLowerCase();
      return components.find((c) =>
        getCategoryNameForComponent(c).toLowerCase().includes(needle),
      );
    }
    return components.find((c) => {
      const catUuid = getCategoryUuidForComponent(c);
  ...
  if (isLoading && components.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full">
        ...
        <p>{t("common.loading", "Loading components...")}</p>
      </div>
    );
  }
    ...
    <div className="flex flex-col h-full bg-gray-50/50" dir={i18n.dir()}>
      <div className="flex items-center justify-between p-4 bg-white border-b sticky top-0 z-20">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-xl font-bold">
              {t("components.title", "Component Selection")}
            </h2>
          </div>
        </div>
        <div className="flex items-center gap-3">
          {!currentInvoice?.issued_at && (
            ...
            >
              {isGenerating ? (
                <Spinner className=" h-4 w-4 group-hover:invert" />
              ) : (
                <img
                  src="eva-icons (2)/outline/bulb.png"
      ...
      <ScrollArea className="flex-grow p-6">
        <div className="max-w-5xl mx-auto space-y-8">
          ...
          {!currentInvoice?.issued_at && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <ComponentSlot
                ...
                requirements={{
                  display: reqInverter
                    ? `${reqInverter.recommended_rating} W` +
                      `, ${reqInverter.output_voltage_v} V`
                    : "N/A",
                ...
                onSelect={() => {
                  setSelectedSlotCategory("Inverter");
              ...
              <ComponentSlot
                ...
                requirements={{
                  display: reqBattery
                    ? `${reqBattery.capacity_per_unit_ah} Ah` +
                      ` x ${reqBattery.quantity}`
                    : "N/A",
                ...
                onDraftCommit={handleDraftCommit}
                onSelect={() => {
                  setSelectedSlotCategory("Battery");
                  setIsInventoryModalOpen(true);
                }}
                onRemove={handleRemove}
              />
              <ComponentSlot
                ...
                requirements={{
                  display: reqPanels
                    ? `${reqPanels.power_rating_w} W` +
                      ` x ${reqPanels.quantity}`
                    : "N/A",
          ...
          >
            <div className="p-4 border-b bg-gray-50 flex items-center justify-between">
              <h3 className="font-bold text-lg">
                {t(
                  "components.system_components_table",
              ...
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-right w-[150px]">
                    {t("components.unit_price", "Unit Price")}
                  </TableHead>
                  <TableHead className="text-right w-[150px]">
                    {t("components.total", "Total")}
                  </TableHead>
              ...
              <TableBody>
                {components.length === 0 ? (
                  ...
                  components.map((c) => (
                    <TableRow
                      key={c.uuid}
                      className={cn(c.is_recommended && "bg-blue-50/30")}
                    >
                      <TableCell>
                        ...
                        <div className="flex items-center justify-center gap-2">
                          {currentInvoice?.issued_at ? (
                            ...
                            <Input
                              ...
                              value={
                                quantityDrafts[c.uuid] ??
                                String(c.quantity ?? 1)
                              }
                              onChange={(e) =>
                                handleDraftChange(c.uuid, e.target.value)
                              }
          ...
          {/* Stock Alert */}
          {components.some(
            (c) => c.item && c.item.quantity_on_hand < c.quantity,
          ) && (
            <Alert variant="destructive" dir={i18n.dir()}>
              <AlertTitle>
                {t("components.stock_warning_title", "Insufficient Stock")}
              </AlertTitle>
              <AlertDescription>
                {t(
                  "components.stock_warning_desc",
                  "Some selected items exceed current inventory levels. Please adjust quantities or check stock.",
                )}
              </AlertDescription>
            </Alert>
          )}
      ...
      >
        <InventorySelectorModal
          ...
          selectedItemUuid={
            selectedSlotCategory === "Inverter"
              ? slotInverter?.item_uuid
              : selectedSlotCategory === "Battery"
                ? slotBattery?.item_uuid
                : selectedSlotCategory === "Panel"
                  ? slotPanels?.item_uuid
                  : undefined
          }
        />
      </Dialog>
...
function ComponentSlot({
  ...
  const isUnderstocked =
    component?.item && component.item.quantity_on_hand < component.quantity;
  ...
  const getStatus = () => {
    if (!component)
      return {
        status: "empty",
        msg: t(
          "components.status.empty",
          "No item selected to fulfill requirements.",
        ),
    ...
    const msgs: string[] = [];
    let status: "fulfilled" | "misaligned" | "understocked" | "over" =
      "fulfilled";
    ...
    if (requirements.primaryValue && itemValue < requirements.primaryValue) {
      ...
      msgs.push(
        t(
          "components.status.spec_below",
          "Spec below requirement ({{val}} {{unit}})",
          { val: itemValue, unit: requirements.primaryUnit },
        ),
      );
    }
    ...
    if (requirements.quantity && component.quantity < requirements.quantity) {
      ...
      msgs.push(
        t(
          "components.status.qty_below",
          "Quantity below requirement ({{qty}}/{{req}})",
          { qty: component.quantity, req: requirements.quantity },
        ),
      );
    }
    ...
    if (isUnderstocked) {
      ...
      msgs.push(
        t(
          "components.status.stock_insufficient",
          "Insufficient stock ({{count}} available)",
          { count: component.item?.quantity_on_hand },
        ),
      );
    }
    ...
    >
      ...
      <div className="flex items-center gap-3 mb-3">
        ...
        <div className="flex-grow">
          ...
          <p className="text-[10px] text-muted-foreground text-semantic-error font-bold uppercase">
            {t("components.required", "Required")}: {requirements.display}
          </p>
        </div>
      </div>
      ...
      <div className="flex-grow flex flex-col">
        {component ? (
          <div className="space-y-3 flex-grow flex flex-col">
            <div className="p-3 bg-blue-50/30 rounded-lg border border-blue-100 flex-grow">
              ...
              <div className="text-xs text-muted-foreground line-clamp-1">
                {component.item?.brand} | {component.item?.model}
              </div>

              <div className="mt-4 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <span className="text-xs font-bold">
                    {t("components.qty", "Qty")}:
                  </span>
                  <Input
                    type="number"
                    className="w-14 h-7 text-center text-xs px-1"
                    value={
                      quantityDrafts[component.uuid] ??
                      String(component.quantity ?? 1)
                    }
                    onChange={(e) =>
                      onDraftChange(component.uuid, e.target.value)
                    }
                ...
                <div className="text-sm font-bold text-blue-700">
                  {formatCurrency(
                    (component.price_at_sale || 0) * component.quantity,
                  )}
                </div>
              </div>
            </div>
            ...
            <div
              className={cn(
                "flex items-center gap-1.5 p-2 rounded text-[11px] content-start font-bold leading-tight",
                statusInfo.status === "fulfilled"
                  ? "bg-green-50"
                  : statusInfo.status === "over"
                    ? "bg-blue-50"
                    : statusInfo.status === "misaligned"
                ...
                component && (
                  <TooltipProvider>
                    <Tooltip>
                      ...
                      <TooltipContent
                        className="bg-white border p-3 shadow-xl w-fit"
                        dir={i18n.dir()}
                      >
                        <div className="space-y-2 text-xs">
                          <p className="font-bold border-b pb-1">
                            {t(
                              "components.spec_comparison",
                              "Technical Comparison",
                            )}
                          </p>
                          <div className="grid grid-cols-2 gap-x-4">
                            <span className="text-gray-500">
                              {t("components.requirement", "Requirement")}:
                            </span>
                            ...
                            <span className="text-gray-500">
                              {t("components.selected_item", "Selected Item")}:
                            </span>
                            <span className="font-semibold">
                              {statusInfo.details?.itemValue}
                              {requirements.primaryUnit}
                              {statusInfo.details?.itemVoltage
                                ? `, ${statusInfo.details.itemVoltage}V`
                                : ""}
            ...
            <div className="flex gap-2 mt-auto">
              <Button
                variant="outline"
                size="sm"
                className="flex-grow h-8 text-xs font-semibold"
                onClick={onSelect}
              >
              ...
              <Button
                variant="outline"
                size="sm"
                className="h-8 w-8 p-0 text-red-500 hover:text-red-600 border-red-100"
          ...
          <div className="flex-grow flex flex-col items-center justify-center py-6 border-2 border-dashed border-gray-100 rounded-lg bg-gray-50/50">
            ...
            <Button
              size="sm"
              className="h-8 font-bold bg-white text-blue-600 border-blue-200 hover:bg-blue-50 shadow-none"
              variant="outline"
              onClick={onSelect}
            >
              <Plus className="h-3 w-3 me-1" />{" "}
              {t("components.select_item", "Select Item")}
            </Button>
          </div>
        )}
      </div>
