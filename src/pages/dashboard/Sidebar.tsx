...
const SidebarItem = ({
  ...
  const content = (
    ...
    >
      {icon !== "sales" ? (
        ...
        <img
          src="/eva-icons (2)/outline/banknote.png"
          alt=""
          className={`w-6 h-6 opacity-40 group-hover:opacity-100`}
        />
      )}
  ...
  return (
    <Button
      variant="ghost"
      dir={i18n.dir()}
      className={`w-full group justify-start gap-4 px-4 h-12 text-md rounded-lg hover:bg-white hover:shadow-sm ${isSelected ? "bg-white shadow-sm [&_img]:opacity-100" : ""}`}
      onClick={onClick}
    >
      {content}
    </Button>
...
const SubscriptionDetails = ({ onOpenModal }: { onOpenModal: () => void }) => {
      ...
      <PopoverContent className="w-80 bg-white" dir={i18n.dir()}>
        <div className="flex flex-col gap-2">
          <h4 className="font-bold">
            {t("dashboard.organization_plan", "Organization Plan")}
          </h4>
          <p className="text-sm text-muted-foreground">
            {t(
              "dashboard.org_plan_desc",
              "You are subscribed to the organization plan.",
            )}
          </p>
  ...
  return (
    <PopoverContent
      className={`w-80 bg-white ${i18n.dir() === "ltr" ? "translate-x-20" : "-translate-x-20"} -translate-y-20`}
      dir={i18n.dir()}
    >
      <div className="grid gap-4">
        <div className="space-y-2">
          <h2 className="text-sm text-muted-foreground">
            {t(
              "dashboard.sub_details_desc",
              "Your current subscription details.",
            )}
          </h2>
        </div>
        {(accountType === "standard" ||
          accountType?.startsWith("enterprise")) && (
          <div className="grid gap-2">
            ...
            {isGrace && currentSubscription?.grace_period_end && (
              <div className="text-[10px] text-yellow-700 bg-yellow-50 p-2 rounded border border-yellow-100 flex items-start gap-1">
                ...
                <span>
                  {t("dashboard.grace_warning", "Grace period ends on")}{" "}
                  {format(currentSubscription.grace_period_end, "dd/MM/yyyy")}
                </span>
              </div>
            )}
            <Button className="mt-2 w-full text-white" onClick={onOpenModal}>
              {currentUser?.status === "active"
                ? t("dashboard.plan_details", "View Plan Details")
                : t("dashboard.renew_upgrade", "Renew / Upgrade")}
            </Button>
          </div>
        )}
        {currentSubscription?.type === "lifetime" && (
          <div className="grid gap-2 text-center p-4 bg-gray-100 rounded-md">
            <span className="font-bold">
              {t("dashboard.lifetime_license", "Lifetime License")}
            </span>
          </div>
        )}
      </div>
    ...
    <>
      <Dialog onOpenChange={(open) => !open && setNonNavSelected(null)}>
        ...
        >
          {/* Header */}
          <div
            className={`flex items-center p-4 h-20 ${showSidebarContent ? "justify-between" : "justify-center"}`}
          >
            ...
            {showSidebarContent && (
              <Button
                ...
                className={`group ${isCollapsed ? "hover:bg-gray-700" : "bg-gray-700 hover:bg-white"}`}
              >
                <SidebarClose
                  className={`h-6 w-6 text-white group-hover:text-gray-700 ${i18n.dir() === "rtl" && "rotate-180"}`}
                />
              </Button>
            )}
          </div>
          ...
          <nav className="flex-grow px-2 space-y-2 pt-4">
            <div className="space-y-1">
              <SidebarItem
                ...
                icon="/eva-icons (2)/outline/grid.png"
                text={t("dashboard.dashboard", "Dashboard")}
                to="/home/dashboard"
                isSelected={nonNavSelected !== null}
              />
              <SidebarItem
                showSidebarContent={showSidebarContent}
                icon="/eva-icons (2)/outline/people.png"
                text={t("dashboard.customers", "Customers")}
                to="/home/customers"
                isSelected={nonNavSelected !== null}
              />
            </div>
            ...
            <div className="space-y-1">
              <SidebarItem
                showSidebarContent={showSidebarContent}
                icon="/eva-icons (2)/outline/archive.png"
                text={t("dashboard.inventory", "Inventory")}
                to="/home/inventory"
                isSelected={nonNavSelected !== null}
              />
              <SidebarItem
            ...
            <div className="space-y-1">
              {currentUser?.role === "admin" && (
                <SidebarItem
                  showSidebarContent={showSidebarContent}
                  icon="/eva-icons (2)/outline/building.png"
                  text={t("dashboard.team", "Team / Organization")}
                  to="/home/team"
                  isSelected={nonNavSelected !== null}
                />
              )}
              <Popover
                onOpenChange={(open) => !open && setNonNavSelected(null)}
              >
                <PopoverTrigger asChild>
                  <div onClick={() => setNonNavSelected("subscription")}>
                    <SidebarItem
                      showSidebarContent={showSidebarContent}
                      icon="/eva-icons (2)/outline/credit-card.png"
                      text={t("dashboard.subscription", "My Plan")}
                      isSelected={nonNavSelected === "subscription"}
                    />
                  </div>
                </PopoverTrigger>
                <SubscriptionDetails
                  onOpenModal={() => setIsPlanModalOpen(true)}
                />
              </Popover>
            </div>
            <Separator className="bg-gray-700 my-2" />
              ...
              <Dialog onOpenChange={(open) => !open && setNonNavSelected(null)}>
                <DialogTrigger asChild>
                  <div onClick={() => setNonNavSelected("support")}>
                    <SidebarItem
                      showSidebarContent={showSidebarContent}
                      icon="/eva-icons (2)/outline/question-mark-circle.png"
                      text={t("dashboard.support", "Support")}
                      isSelected={nonNavSelected === "support"}
                    />
                  </div>
                </DialogTrigger>
                <SupportModal />
              ...
              <Dialog onOpenChange={(open) => !open && setNonNavSelected(null)}>
                <DialogTrigger asChild>
                  <div onClick={() => setNonNavSelected("settings")}>
                    <SidebarItem
                      showSidebarContent={showSidebarContent}
                      icon="/eva-icons (2)/outline/settings-2.png"
                      text={t("dashboard.settings", "Settings")}
                      isSelected={nonNavSelected === "settings"}
                    />
                  </div>
                </DialogTrigger>
                <SettingsModal passwordChange={false} />
              </Dialog>
            </div>
          </nav>
          ...
          <div className="p-4">
            ...
            >
              {isSyncing ? (
                ...
                <img
                  src="/eva-icons (2)/outline/sync.png"
                  alt={t("common.synced_alt", "synced")}
                  className="w-6 h-6 opacity-40 group-hover:opacity-100"
                />
              )}
              {showSidebarContent && (
