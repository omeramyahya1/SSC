...
export function CreateProjectModal({
  ...
  const cities = useMemo(
    () => getCitiesByState(locationState),
    [locationState, getCitiesByState],
  );

  useEffect(() => {
    if (
      initialData?.bleSettings?.project_location_state &&
      initialData?.bleSettings?.project_location_city
    ) {
      ...
      const [city, state] = initialData.config.metadata.location
        .split(", ")
        .map((s) => s.trim());
  ...
  const handleSelectExistingCustomer = (id: string) => {
    ...
    const selected = customers.find(
      (c) => String(c.customer_id) === id || c.uuid === id,
    );
    if (!selected) return;
  ...
  const handleCreate = () => {
    const projectData: NewProjectData = {
      customer_uuid: useExistingCustomer
    ...
    <DialogContent className="sm:max-w-[525px] bg-white" dir={i18n.dir()}>
      <DialogHeader>
        <DialogTitle className="text-2xl font-bold">
          {t("dashboard.create_project_title", "Create a New Project")}
        </DialogTitle>
      </DialogHeader>
      <div className="grid gap-6 py-4">
        <div className="flex items-center justify-between gap-3 rounded-lg border border-gray-200 bg-gray-50 px-3 py-2">
          <Label htmlFor="use-existing-customer" className="font-semibold">
            {t(
              "dashboard.use_existing_customer",
            ...
            <SearchableSelect
              ...
              placeholder={
                customers.length == 0
                  ...
                  : t("dashboard.existing_customer_ph", "Select a customer...")
              }
      ...
      <DialogFooter className="flex gap-4 sm:justify-end">
        ...
        >
          {t("dashboard.create_button", "Create Project")}
        </Button>
      </DialogFooter>
    </DialogContent>
  );
