...
function ProjectInfo({
  ...
  useEffect(() => {
    if (project && !isEditing) {
      const state =
        project.project_location?.split(", ").map((p) => p.trim())[1] || "";
      setEditData({
  ...
  const handleFieldChange = (
    field: keyof ProjectUpdatePayload,
    value: string,
  ) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.error(
        t("project_modal.update_error", "Failed to update project details."),
      );
    }
  };
    ...
    <div className="relative border rounded-lg p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
          ...
          {isEditing ? (
            ...
            <p className="text-base font-semibold">
              {project.customer?.full_name ??
                t("dashboard.no_customer", "No Customer")}
            </p>
          )}
        </div>
        ...
        <div>
          ...
          {isEditing ? (
            <SearchableSelect
              items={states.map((s) => ({ label: s.label, value: s.value }))}
              value={selectedState}
        ...
        <div>
          ...
          {isEditing ? (
            <SearchableSelect
              ...
              value={editData.project_location?.split(", ")[0] || ""}
...
export function ProjectDetailsModal({
  ...
  const [currentView, setCurrentView] = useState<
    "config" | "components" | "invoicing"
  >("config");
  const [isEditingAppliances, setIsEditingAppliances] = useState(false);

  const { currentInvoice, fetchInvoiceByProject, setCurrentInvoice } =
    useInvoiceStore();
  ...
  const localInvoice =
    currentInvoice?.project_uuid === projectProp?.uuid
      ? currentInvoice
      : undefined;
  const isInvoiceIssued = !!localInvoice?.issued_at;
  const isViewOnlyProject = project?.access?.mode === "view";
  const isReadOnlyProject = isInvoiceIssued || isViewOnlyProject;
  ...
  const handleStatusChange = async (newStatus: Project["status"]) => {
    ...
    try {
      ...
      setProject((prev) =>
        prev ? { ...prev, status: newStatus, is_pending: false } : null,
      );
      toast.success(
        t("project_modal.status_update_success", "Project status updated!"),
      );
    } catch (error) {
      toast.error(
        t("project_modal.status_update_error", "Failed to update status."),
      );
      console.error(error);
    }
  };
  ...
  const [customApplianceWattage, setCustomApplianceWattage] =
    useState<number>(0);
  ...
  const [bleSettingsErrors, setBleSettingsErrors] = useState<{
  ...
  useEffect(() => {
    if (systemConfiguration?.config_items) {
      ...
      setBleSettings((prev) => ({
        ...prev,
        inverter_efficiency: config.inverter?.efficiency_percent
          ? config.inverter.efficiency_percent / 100
          : prev.inverter_efficiency,
        inverter_rated_power:
          config.inverter?.power_rating_w || prev.inverter_rated_power,
        ...
        battery_dod: config.battery_bank?.depth_of_discharge_percent
          ? config.battery_bank.depth_of_discharge_percent / 100
          : prev.battery_dod,
        panel_rated_power:
          config.solar_panels?.power_rating_w || prev.panel_rated_power,
      }));
    }
  }, [systemConfiguration]);
  ...
  const validateBleSetting = (key: string, value: any): string | null => {
    ...
    switch (key) {
      ...
      case "system_losses":
        if (value < 0 || value > 1)
          error = t("common.must_be_between_0_1", "Must be between 0 and 1");
        break;
      case "safety_factor":
        if (value < 1)
          error = t("common.must_be_1_or_greater", "Must be 1 or greater");
        break;
      ...
      case "reference_irradiance":
        if (value <= 0)
          error = t(
            "common.must_be_positive_number",
      ...
      case "temp_coefficient_power":
        if (value > 0 || value < -1)
          error = t(
            "common.temp_coefficient_power_hint",
            "Typically a small negative number (e.g., -0.004)",
          );
        break;
    }
    return error;
  ...
  const [applianceInputErrors, setApplianceInputErrors] = useState<{
    [key: string]: { [field: string]: string | null };
  }>({});

  const validateApplianceInput = (
    field: "qty" | "use_hours_night" | "wattage",
    value: number,
  ): string | null => {
    let error: string | null = null;
    if (isNaN(value)) {
  ...
  const handleUpdateAppliance = async (
    appliance_id: number,
    updates: Partial<Omit<ProjectAppliance, "appliance_id">>,
  ) => {
    if (isArchived) return;
    let hasError = false;
    ...
    for (const key in updates) {
      if (key === "qty" || key === "use_hours_night" || key === "wattage") {
        ...
        const error = validateApplianceInput(
          key as "qty" | "use_hours_night" | "wattage",
          value,
        );
        if (error) {
  ...
  const totalEnergy = useMemo(
    () =>
      projectAppliances.reduce(
        (sum, app) => sum + calculateApplianceMetrics(app).energy,
        0,
      ),
    [projectAppliances],
  );
  const totalPower = useMemo(
    () =>
      projectAppliances.reduce(
        (sum, app) => sum + calculateApplianceMetrics(app).power,
        0,
      ),
    [projectAppliances],
  );
  ...
  const handleSaveConfiguration = async () => {
    ...
    try {
      ...
      toast.success(
        t(
          "project_modal.config_save_success",
      ...
      toast.error(
        t(
          "project_modal.config_save_error",
  ...
  if (currentView === "components" && project.uuid) {
    return (
      <DialogContent
        className="max-w-[95vw] h-[95vh] flex flex-col p-0 bg-white"
        dir={i18n.dir()}
      >
        <ComponentSelectionView
          ...
          bleResults={
            bleResults?.data
              ? bleResults
              : { data: systemConfiguration?.config_items }
          }
    ...
    return (
      ...
      >
        <InvoiceEditor
          ...
          onBack={() =>
            setCurrentView(
              currentInvoice?.issued_at && true ? "config" : "components",
            )
          }
        />
      </DialogContent>
  ...
  return (
    <DialogContent
      className="max-w-[90vw] h-[90vh] flex flex-col p-0 bg-white"
      dir={i18n.dir()}
    >
        ...
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          >
            {!isViewOnlyProject && (
              <SelectTrigger
                className={cn(
                  `w-[100px] border px-2 py-1 rounded-xl text-xs font-semibold flex justify-center items-center gap-1`,
                  statusColors[project.status] || "bg-gray-100",
                  project.is_pending && "animate-pulse",
                )}
              >
                <SelectValue>
                  {project.is_pending
                    ? t("dashboard.pending", "Saving...")
            ...
            <SelectContent>
              <SelectItem value="planning">
                {t("dashboard.status.planning", "Planning")}
              </SelectItem>
              <SelectItem value="execution">
                {t("dashboard.status.execution", "Execution")}
              </SelectItem>
              <SelectItem value="done">
                {t("dashboard.status.done", "Done")}
              </SelectItem>
              <SelectItem value="archived">
                {t("dashboard.status.archived", "Archived")}
              </SelectItem>
            </SelectContent>
          </Select>
        </div>
        ...
        <div className="flex flex-col p-6 overflow-y-auto border-e gap-6">
          ...
          <div>
            <h3 className="flex flex-row gap-2 text-xl font-bold mb-4">
              ...
              <span>
                {t(
                  "project_modal.customer_info_location",
                  "Customer Info & Location",
                )}
              </span>
            </h3>
            <ProjectInfo
              project={project}
              onUpdate={setProject}
              isReadOnly={isArchived || isReadOnlyProject}
              issued={Boolean(currentInvoice?.issued_at && true)}
            />
          </div>
              ...
              <Accordion type="single" collapsible defaultValue="">
                <AccordionItem value="ble-settings">
                  <AccordionTrigger className="text-xl font-bold flex flex-row w-full">
                    ...
                    <span>
                      {t(
                        "project_modal.design_parameters",
                        "System Design Parameters",
                      )}
                    </span>
                  </AccordionTrigger>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-4 pt-4">
                      ...
                      <div className="space-y-3 p-3 bg-gray-50 rounded-md border">
                        <h4 className="font-semibold text-gray-700">
                          {t("project_modal.inverter", "Inverter")}
                        </h4>
                        <SettingsInput
                          label={t(
                            "ble.inverter.rated_power_label",
                            "Rated Power (W)",
                          )}
                          value={bleSettings.inverter_rated_power}
                          onChange={(v) =>
                            handleBleSettingChange("inverter_rated_power", v)
                          }
                          step={100}
                          min={1}
                        ...
                        <SettingsInput
                          label={t(
                            "ble.inverter.efficiency_label",
                            "Efficiency (%)",
                          )}
                          value={bleSettings.inverter_efficiency * 100}
                          onChange={(v) =>
                            handleBleSettingChange(
                              "inverter_efficiency",
                              Number(v) / 100,
                            )
                          }
                        ...
                        <SettingsInput
                          label={t(
                            "ble.inverter.safety_factor_label",
                            "Safety Factor (%)",
                          )}
                        ...
                        <SettingsInput
                          label={t(
                            "ble.inverter.mppt_min_v_label",
                            "MPPT Min Voltage (V)",
                          )}
                          value={bleSettings.inverter_mppt_min_v}
                          onChange={(v) =>
                            handleBleSettingChange("inverter_mppt_min_v", v)
                          }
                          step={1}
                          min={1}
                        ...
                        <SettingsInput
                          label={t(
                            "ble.inverter.mppt_max_v_label",
                            "MPPT Max Voltage (V)",
                          )}
                          value={bleSettings.inverter_mppt_max_v}
                          onChange={(v) =>
                            handleBleSettingChange("inverter_mppt_max_v", v)
                          }
                          step={1}
                          min={1}
                      ...
                      <div className="space-y-3 p-3 bg-gray-50 rounded-md border">
                        <h4 className="font-semibold text-gray-700">
                          {t("project_modal.battery_bank", "Battery Bank")}
                        </h4>
                        <div>
                          <Label className="text-xs font-medium text-gray-600">
                            {t(
                              "project_modal.battery_type_label",
                              "Battery Type",
                            )}
                          </Label>
                          <Select
                          ...
                          >
                            ...
                            <SelectContent>
                              ...
                              <SelectItem value="other">
                                {t("project_modal.battery_type.other", "Other")}
                              </SelectItem>
                            </SelectContent>
                          </Select>
                        </div>
                        <SettingsInput
                          label={t(
                            "ble.battery_bank.capacity_per_unit_label",
                            "Capacity per Unit (Ah)",
                          )}
                        ...
                        <SettingsInput
                          label={t(
                            "ble.battery_bank.voltage_per_unit_label",
                            "Voltage per Unit (V)",
                          )}
                          value={bleSettings.battery_rated_voltage}
                          onChange={(v) =>
                            handleBleSettingChange("battery_rated_voltage", v)
                          }
                          step={1}
                          min={1}
                        ...
                        <SettingsInput
                          label={t(
                            "ble.battery_bank.efficiency_label",
                            "Efficiency (%)",
                          )}
                        ...
                        <SettingsInput
                          label={t(
                            "ble.battery_bank.max_parallel_units_label",
                            "Max Parallel Units",
                          )}
                          value={bleSettings.battery_max_parallel}
                          onChange={(v) =>
                            handleBleSettingChange("battery_max_parallel", v)
                          }
                          step={1}
                          min={1}
                        ...
                        <SettingsInput
                          label={t(
                            "ble.battery_bank.autonomy_days_label",
                            "Days of Autonomy",
                          )}
                          value={bleSettings.autonomy_days}
                          onChange={(v) =>
                            handleBleSettingChange("autonomy_days", v)
                          }
                          step={1}
                          min={1}
                      ...
                      <div className="space-y-3 p-3 bg-gray-50 rounded-md border">
                        <h4 className="font-semibold text-gray-700">
                          {t("project_modal.solar_array", "Solar Array")}
                        </h4>
                        <SettingsInput
                          label={t(
                            "ble.solar_panels.panel_power_label",
                            "Panel Power (W)",
                          )}
                          value={bleSettings.panel_rated_power}
                          onChange={(v) =>
                            handleBleSettingChange("panel_rated_power", v)
                          }
                          step={10}
                          min={1}
                        ...
                        <SettingsInput
                          label={t(
                            "ble.solar_panels.panel_mpp_voltage_label",
                            "Panel MPP Voltage (V)",
                          )}
                          value={bleSettings.panel_mpp_voltage}
                          onChange={(v) =>
                            handleBleSettingChange("panel_mpp_voltage", v)
                          }
                          step={0.1}
                          min={1}
                        ...
                        <SettingsInput
                          label={t(
                            "ble.solar_panels.system_losses_label",
                            "System Losses (%)",
                          )}
                        ...
                        <SettingsInput
                          label={t(
                            "ble.solar_panels.temp_coefficient_power_label",
                            "Temp Coefficient Power",
                          )}
                          value={bleSettings.temp_coefficient_power}
                          onChange={(v) =>
                            handleBleSettingChange("temp_coefficient_power", v)
                          }
                          step={0.001}
                          min={-1}
                          max={0}
                        ...
                        <SettingsInput
                          label={t(
                            "ble.solar_panels.stc_temp_label",
                            "STC Temp (°C)",
                          )}
                          value={bleSettings.stc_temp}
                          onChange={(v) =>
                            handleBleSettingChange("stc_temp", v)
                          }
                          step={1}
                          min={0}
                        ...
                        <SettingsInput
                          label={t(
                            "ble.solar_panels.reference_irradiance_label",
                            "Reference Irradiance (W/m²)",
                          )}
                          value={bleSettings.reference_irradiance}
                          onChange={(v) =>
                            handleBleSettingChange("reference_irradiance", v)
                          }
                          step={1}
                          min={1}
                        ...
                        >
                          ...
                          />
                          <Label
                            htmlFor="calculate-temp-derating"
                            className="text-xs font-medium text-gray-600"
                          >
                            {t(
                              "project_modal.calculate_temp_derating_label",
                              "Calculate Temp Derating",
                            )}
                          </Label>
                        </div>
                      </div>
          ...
          <div>
            <h3 className="flex flex-row gap-2 text-xl font-bold mb-4">
              ...
              <span>
                {t(
                  "project_modal.appliances_breakdown",
            ...
            {!isArchived && (!isReadOnlyProject || isEditingAppliances) && (
              <div className="grid grid-cols-12 gap-2 p-3 mb-4 border rounded-lg bg-gray-50">
                <div className="col-span-7">
                  ...
                  >
                    {t("project_modal.appliance_name_label", "Appliance Name")}
                  </Label>
                  <Input
                    id="appliance-name"
                    placeholder={t(
                      "project_modal.add_appliance_name_ph",
                      "e.g., Refrigerator",
                    )}
                ...
                <div className="col-span-3">
                  <Label
                    htmlFor="appliance-wattage"
                    className="text-xs text-gray-500 font-semibold"
                  >
                    {t("project_modal.wattage_label", "Wattage (W)")}
                  </Label>
                  <Input
                    id="appliance-wattage"
                    type="number"
                    placeholder={t(
                      "project_modal.add_appliance_wattage_ph",
                      "e.g., 150",
                    )}
                    value={customApplianceWattage}
                    onChange={(e) =>
                      setCustomApplianceWattage(Number(e.target.value))
                    }
                    className="bg-white"
                  />
                </div>
                <div className="col-span-2 flex items-end">
                  <Button
                    ...
                    disabled={
                      !customApplianceName ||
                      !customApplianceWattage ||
                      customApplianceWattage <= 0
                    }
                  ...
                  >
                    ...
                    />{" "}
                    {t("project_modal.add", "Add")}
                  </Button>
                </div>
              </div>
            )}

            {isApplianceLoading && projectAppliances.length === 0 && (
            ...
            {applianceError && (
              <Alert variant="destructive">
                ...
                <AlertDescription>{applianceError}</AlertDescription>
              </Alert>
            )}
            ...
            <ScrollArea className="flex-grow h-[300px] mb-4 rounded-md border">
              <Table dir={i18n.dir()} className="overflow-x-scroll">
                <TableHeader>
                  <TableRow>
                    <TableHead className="text-start w-[150px] font-bold">
                      {t("project_modal.appliance", "Appliance")}
                    </TableHead>
                    <TableHead className="text-start w-[100px] font-bold">
                      {t("project_modal.wattage", "Wattage (W)")}
                    </TableHead>
                    <TableHead className="text-start w-[80px] font-bold">
                      {t("project_modal.qty", "Qty")}
                    </TableHead>
                    <TableHead className="text-start w-[100px] font-bold">
                      {t("project_modal.use_hours_night", "Night/Battery Hrs")}
                    </TableHead>
                    <TableHead className="text-start w-[100px] font-bold">
                      {t("project_modal.power", "Power (W)")}
                    </TableHead>
                    <TableHead className="text-start w-[120px] font-bold">
                      {t("project_modal.energy", "Energy (Wh/day)")}
                    </TableHead>
                    {!isArchived &&
                      (!isReadOnlyProject || isEditingAppliances) && (
                  ...
                  {projectAppliances.map((app) => (
                    <TableRow key={app.appliance_id}>
                        ...
                        {!isReadOnlyProject || isEditingAppliances ? (
                          <div className="flex flex-col">
                            <Input
                              ...
                              value={app.wattage}
                              onChange={(e) =>
                                handleUpdateAppliance(app.appliance_id, {
                                  wattage: parseFloat(e.target.value) || 0,
                                })
                              }
                              className={cn(
                                "w-full text-center p-1 h-8",
                                applianceInputErrors[app.appliance_id]
                                  ?.wattage && "border-red-500",
                              )}
                              disabled={isArchived}
                            />
                            {applianceInputErrors[app.appliance_id]
                        ...
                        {!isReadOnlyProject || isEditingAppliances ? (
                          <div className="flex flex-col">
                            <Input
                              ...
                              className={cn(
                                "w-full text-center p-1 h-8",
                                applianceInputErrors[app.appliance_id]?.qty &&
                                  "border-red-500",
                              )}
                              disabled={isArchived}
                            />
                          ...
                          <div className="flex flex-col">
                            <Input
                              ...
                              className={cn(
                                "w-full text-center p-1 h-8",
                                applianceInputErrors[app.appliance_id]
                                  ?.use_hours_night && "border-red-500",
                              )}
                              disabled={
                                isArchived || bleSettings.autonomy_days === 0
                              }
                            />
                            {applianceInputErrors[app.appliance_id]
                              ?.use_hours_night && (
                      ...
                      <TableCell className="text-center">
                        {calculateApplianceMetrics(app).power}
                      </TableCell>
                      <TableCell className="text-center">
                        {calculateApplianceMetrics(app).energy}
                      </TableCell>
                      {!isArchived &&
                        (!isReadOnlyProject || isEditingAppliances) && (
                          <TableCell>
                            <Button
                              ...
                              size="icon"
                              onClick={() =>
                                removeApplianceFromProject(app.appliance_id)
                              }
                            >
            ...
            <div className="flex flex-row justify-between items-center px-4 py-2 border-t font-bold text-end">
              <span className="text-transparent">
                {t("project_modal.total_power", "Power")}:{" "}
              ...
              <span>
                {t("project_modal.total_energy", "Energy")}:{" "}
                {formatEnergyValue(totalEnergy)}/day
              </span>
            </div>
          </div>

          {!isArchived && !isReadOnlyProject && (
            <Button
              onClick={handleRunCalculation}
              disabled={
                isBleLoading ||
                hasBleSettingsErrors ||
                hasApplianceInputErrors ||
        ...
        <div className="flex flex-col p-6 overflow-y-auto">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-xl font-bold">
              {t("project_modal.system_configuration", "System Configuration")}
            </h3>
          </div>
          ...
          {!isResultsLoading && !resultsError && !displayResults && (
            <div className="flex flex-col items-center justify-center h-full text-center text-gray-500">
              ...
              <p className="font-semibold">
                {t("project_modal.no_results_title", "No Results Yet")}
              </p>
              <p className="text-sm">
                {t(
                  "project_modal.no_results_subtitle",
                  'Click "Calculate" to run the system analysis.',
                )}
              </p>
            </div>
          )}
            ...
            <ScrollArea className="flex-grow">
              ...
              >
                <AccordionItem value="metadata">
                  <AccordionTrigger
                    className={cn(
                      "font-bold flex w-full",
                      i18n.dir() === "rtl" ? "flex-row-reverse" : "",
                    )}
                  >
                    <Info className="h-4 w-4" />
                    <span>
                      {t("project_modal.metadata_accordion_title", "Metadata")}
                    </span>
                  </AccordionTrigger>
                  <AccordionContent>
                    <div className="grid grid-cols-2 gap-x-4 gap-y-2">
                      <DataRow
                        label={t(
                          "ble.metadata.peak_sun_hours",
                          "Peak Sun Hours",
                        )}
                        value={displayResults.metadata.peak_sun_hours}
                      />
                      <DataRow
                        label={t(
                          "ble.metadata.total_system_size",
                          "Total System Size",
                        )}
                        value={parseFloat(
                          String(displayResults.metadata.total_system_size_kw),
                        )}
                        unit="kW"
                      ...
                      <DataRow
                        label={t(
                          "ble.metadata.peak_surge_power",
                          "Peak Surge Power",
                        )}
                        value={displayResults.metadata.peak_surge_power_w}
                        formatter={formatPowerValue}
                      />
                      <DataRow
                        label={t("ble.metadata.autonomy_days", "Autonomy Days")}
                        value={displayResults.metadata.autonomy_days}
                      />
                      <DataRow
                        label={t(
                          "ble.metadata.total_daily_energy",
                          "Total Daily Energy",
                        )}
                        value={displayResults.metadata.total_daily_energy_wh}
                        formatter={formatEnergyValue}
                      />
                      <DataRow
                        label={t(
                          "ble.metadata.total_peak_power",
                          "Total Peak Power",
                        )}
                        value={displayResults.metadata.total_peak_power_w}
                        formatter={formatPowerValue}
                      />
                    </div>
                  </AccordionContent>
                ...
                <AccordionItem value="solar_panels">
                  ...
                  >
                    ...
                    <span>
                      {t("project_modal.solar_panels", "Solar Panels")}
                    </span>
                  </AccordionTrigger>
                  <AccordionContent>
                    <div className="grid grid-cols-2 gap-x-4 gap-y-2">
                      <DataRow
                        label={t(
                          "ble.solar_panels.power_rating",
                          "Power Rating",
                        )}
                        value={displayResults.solar_panels.power_rating_w}
                        formatter={formatPowerValue}
                      />
                      <DataRow
                        label={t("ble.solar_panels.quantity", "Quantity")}
                        value={displayResults.solar_panels.quantity}
                      />
                      <DataRow
                        label={t(
                          "ble.solar_panels.total_capacity",
                          "Total PV Capacity",
                        )}
                        value={parseFloat(
                          String(displayResults.metadata.total_system_size_kw),
                      ...
                      <DataRow
                        label={t(
                          "ble.solar_panels.panels_per_string",
                          "Panels per String",
                        )}
                        value={displayResults.solar_panels.panels_per_string}
                      />
                      <DataRow
                        label={t(
                          "ble.solar_panels.num_strings",
                          "Num. Parallel Strings",
                        )}
                        value={displayResults.solar_panels.num_parallel_strings}
                      />
                      <DataRow
                        label={t(
                          "ble.solar_panels.connection",
                          "Connection Type",
                        )}
                        value={displayResults.solar_panels.connection_type}
                      />
                      <DataRow
                        label={t("ble.solar_panels.tilt_angle", "Tilt Angle")}
                        value={`${displayResults.solar_panels.tilt_angle}°`}
                      />
                    </div>
                  </AccordionContent>
                </AccordionItem>
                <AccordionItem value="inverter">
                  <AccordionTrigger
                    className={cn(
                      "font-bold flex w-full justify-start",
                      i18n.dir() === "rtl" ? "flex-row-reverse" : "",
                    )}
                  >
                  ...
                  <AccordionContent>
                    <div className="grid grid-cols-2 gap-x-4 gap-y-2">
                      <DataRow
                        label={t("ble.inverter.power_rating", "Power Rating")}
                        value={displayResults.inverter.power_rating_w}
                        formatter={formatPowerValue}
                      />
                      <DataRow
                        label={t("ble.inverter.quantity", "Quantity")}
                        value={displayResults.inverter.quantity}
                      />
                      <DataRow
                        label={t(
                          "ble.inverter.recommended_rating",
                          "Recommended Rating",
                        )}
                        value={displayResults.inverter.recommended_rating}
                        formatter={formatPowerValue}
                      />
                      <DataRow
                        label={t("ble.inverter.efficiency", "Efficiency")}
                        value={`${displayResults.inverter.efficiency_percent}%`}
                      />
                      <DataRow
                        label={t("ble.inverter.surge_rating", "Surge Rating")}
                        value={displayResults.inverter.surge_rating_w}
                        formatter={formatPowerValue}
                      />
                      <DataRow
                        label={t(
                          "ble.inverter.output_voltage",
                          "Output Voltage",
                        )}
                        value={`${displayResults.inverter.output_voltage_v} V`}
                      />
                      <DataRow
                        label={t("ble.inverter.connection", "Connection Type")}
                        value={displayResults.inverter.connection_type}
                      />
                    </div>
                  </AccordionContent>
                </AccordionItem>
                {displayResults.battery_bank.quantity != 0 && (
                  <AccordionItem value="battery_bank">
                    ...
                    >
                      <BatteryCharging className="h-6 w-6 text-green-500 static" />
                      <span>
                        {t("project_modal.battery_bank", "Battery Bank")}
                      </span>
                    </AccordionTrigger>
                    <AccordionContent>
                      <div className="grid grid-cols-2 gap-x-4 gap-y-2">
                        <DataRow
                          label={t(
                            "ble.battery_bank.battery_type",
                            "Battery Type",
                          )}
                          value={displayResults.battery_bank.battery_type}
                        />
                        <DataRow
                          label={t(
                            "ble.battery_bank.capacity_per_unit",
                            "Capacity per Unit",
                          )}
                          value={`${displayResults.battery_bank.capacity_per_unit_ah} Ah`}
                        />
                        <DataRow
                          label={t(
                            "ble.battery_bank.voltage_per_unit",
                            "Voltage per Unit",
                          )}
                          value={`${displayResults.battery_bank.voltage_per_unit_v} V`}
                        />
                        <DataRow
                          label={t("ble.battery_bank.quantity", "Quantity")}
                          value={displayResults.battery_bank.quantity}
                        />
                        <DataRow
                          label={t(
                            "ble.battery_bank.num_in_series",
                            "Num in Series",
                          )}
                          value={displayResults.battery_bank.num_in_series}
                        />
                        <DataRow
                          label={t(
                            "ble.battery_bank.num_in_parallel",
                            "Num in Parallel",
                          )}
                          value={displayResults.battery_bank.num_in_parallel}
                        />
                        <DataRow
                          label={t(
                            "ble.battery_bank.total_storage",
                            "Total Storage",
                          )}
                          value={parseFloat(
                            String(
                              displayResults.battery_bank.total_storage_kwh,
                            ),
                          )}
                          unit="kWh"
                          formatter={(val: number) => val.toFixed(2)}
                        ...
                        <DataRow
                          label={t(
                            "ble.battery_bank.depth_of_discharge",
                            "Depth of Discharge",
                          )}
                          value={`${displayResults.battery_bank.depth_of_discharge_percent}%`}
                        />
                        <DataRow
                          label={t(
                            "ble.battery_bank.system_voltage",
                            "System Voltage",
                          )}
                          value={`${displayResults.battery_bank.system_voltage_v} V`}
                        />
                        <DataRow
                          label={t(
                            "ble.battery_bank.connection",
                            "Connection Type",
                          )}
                          value={displayResults.battery_bank.connection_type}
                        />
                      </div>
                    </AccordionContent>
                  </AccordionItem>
                    ...
                    <>
                      ...
                      {displayResults && (
                        ...
                        >
                          ...
                          {t(
                            "project_modal.proceed_to_selection",
    ...
    <div className="py-1 border-b border-gray-100">
      {i18n.dir() === "ltr" ? (
        <div className="flex justify-between">
          ...
          <span className="text-sm font-semibold text-gray-800 text-end">
            {displayValue} {unit}
          </span>
        </div>
      ) : (
