...
export function Dashboard() {
  ...
  const { currentUser, users, fetchEmployees } = useUserStore();
  ...
  const [quickCalcData, setQuickCalcData] =
    useState<QuickCalcConvertedData | null>(null);
  ...
  useEffect(() => {
    fetchProjects();
    if (currentUser?.role === "admin" && currentUser?.organization_uuid) {
      fetchEmployees(currentUser.organization_uuid);
      fetchBranches();
    }
  }, [fetchProjects, currentUser, fetchEmployees, fetchBranches]);

  const handleCreateProject = async (
    projectData: NewProjectData,
    quickCalcConvertedData?: QuickCalcConvertedData,
  ) => {
    try {
  ...
  const handleConfirmSingleDelete = async () => {
    ...
    try {
      ...
      toast.success(
        t(
          "dashboard.toast.project_deleted",
  ...
  const handleConfirmEmptyTrash = async () => {
    try {
      await emptyTrash();
      toast.success(
        t("dashboard.toast.trash_emptied", "Trash has been emptied."),
      );
      sync();
    } catch (error) {
      toast.error(
        t(
          "dashboard.toast.empty_trash_failed",
    ...
    return projects
      .filter((p) => {
        ...
        if (searchQuery) {
          ...
          const locationMatch = p.project_location
            ?.toLowerCase()
            .includes(query);
      ...
      .sort((a, b) => {
        ...
        if (sortBy === "location")
          return (a.project_location || "").localeCompare(
            b.project_location || "",
          );
        return 0;
      });
  ...
  const groupedProjects = useMemo(() => {
    ...
    filteredProjects.forEach((p) => {
      let key = "";
      if (groupBy === "status")
        key = t(`dashboard.status.${p.status}`, p.status);
      else if (groupBy === "location")
        key = p.project_location
          ? formatLocation(p.project_location)
          : t("dashboard.no_location", "No Location");
        ...
        const branch = branches.find((b) => b.uuid === p.branch_uuid);
        key = branch ? branch.name : t("dashboard.no_branch", "No Branch");
      } else if (groupBy === "employee") {
        const employee = users.find((u) => u.uuid === p.user_uuid);
        key = employee
          ? employee.username
          : t("dashboard.unknown_employee", "Unknown Employee");
      }
  ...
  const renderContent = () => {
      ...
      return (
        <div className="flex justify-center items-center h-64">
          <Alert variant="destructive" className="max-w-md">
            <AlertTitle>
              {t("dashboard.error_title", "Failed to Load Projects")}
            </AlertTitle>
    ...
    if (filteredProjects.length === 0) {
      const emptyTitle =
        currentView === "trash"
          ? t("dashboard.empty_trash_title", "Trash is Empty")
          : currentView === "archived"
            ? t("dashboard.empty_archive_title", "No Archived Projects")
            : t("dashboard.no_projects_title", "No Projects Found");

      const emptyDesc = searchQuery
        ? t(
            "dashboard.no_search_results",
            "No projects match your search criteria.",
          )
        : currentView === "trash"
          ? t(
              "dashboard.empty_trash_desc",
              "Deleted projects will appear here.",
            )
          : currentView === "archived"
            ? t(
                "dashboard.empty_archive_desc",
      ...
      return (
        <Empty className="mt-5">
          <img
            src="/illustrations/no-projects.png"
            className="w-1/4"
            alt="empty"
          />
            ...
            {currentView === "active" && !searchQuery && (
              ...
              >
                <img
                  ...
                  alt="add"
                  className="w-5 h-5 invert ltr:mr-2 rtl:ml-2"
                />
                <span>
  ...
  return (
    <>
          ...
          <div className="flex flex-col gap-4 mb-6">
            <div className="flex content-end justify-between">
              <div className="flex flex-col gap-4 items-start justify-between h-full">
                ...
                <div className="flex bg-white border rounded-lg p-1 shadow-sm">
                  ...
                  >
                    <img
                      src="/eva-icons (2)/outline/home.png"
                      alt="home"
                      className={`w-4 h-4 ltr:mr-2 rtl:ml-2 ${currentView === "active" ? "invert" : "opacity-70"}`}
                    />
                  ...
                  >
                    <img
                      src="/eva-icons (2)/outline/archive.png"
                      alt="archive"
                      className={`w-4 h-4 ltr:mr-2 rtl:ml-2 ${currentView === "archived" ? "invert" : "opacity-70"}`}
                    />
                  ...
                  >
                    <img
                      src="/eva-icons (2)/outline/trash-2.png"
                      alt="trash"
                      className={`w-4 h-4 ltr:mr-2 rtl:ml-2 ${currentView === "trash" ? "invert" : "opacity-70"}`}
                    />
              ...
              <div className="flex flex-row items-end gap-2">
                ...
                >
                  <img
                    src="/eva-icons (2)/outline/flash.png"
                    alt="quick calc"
                    className="w-5 h-5 group-hover:invert me-2"
                  />
                  <span className="me-2 text-gray-700 group-hover:text-white">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  <SelectTrigger
                    className={`w-auto border-gray-200 flex gap-2 ${sortBy !== "newest" ? "bg-primary text-white" : "bg-white"}`}
                  >
                    <img
                      src="/eva-icons (2)/outline/swap.png"
                      alt="sort"
                      className={`w-4 h-4 rotate-90 ${sortBy !== "newest" ? "invert" : "opacity-60"}`}
                    />
                    <SelectValue
                  ...
                  <SelectContent>
                    <SelectItem value="newest">
                      {t("dashboard.sort.newest", "Newest First")}
                    </SelectItem>
                    <SelectItem value="oldest">
                      {t("dashboard.sort.oldest", "Oldest First")}
                    </SelectItem>
                    <SelectItem value="name-asc">
                      {t("dashboard.sort.name_asc", "Name A-Z")}
                    </SelectItem>
                    <SelectItem value="name-desc">
                      {t("dashboard.sort.name_desc", "Name Z-A")}
                    </SelectItem>
                    <SelectItem value="location">
                      {t("dashboard.sort.location", "Location")}
                    </SelectItem>
                  </SelectContent>
                </Select>
                ...
                {currentView === "active" && (
                  ...
                  >
                    <SelectTrigger
                      className={`w-auto bg-white border-gray-200 flex gap-2 ${filterStatus !== "all" ? "bg-primary text-white" : "bg-white"}`}
                    >
                      <img
                        src="/eva-icons (2)/outline/funnel.png"
                        alt="filter"
                        className={`w-4 h-4 ${filterStatus !== "all" ? "invert" : "opacity-60"}`}
                      />
                      <SelectValue
                        placeholder={t("dashboard.filter", "Filter")}
                      />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="all">
                        {t("dashboard.filter.all", "All Status")}
                      </SelectItem>
                ...
                <Select
                  value={groupBy}
                  onValueChange={(v: GroupOption) => setGroupBy(v)}
                  dir={i18n.dir()}
                >
                  <SelectTrigger
                    className={`w-auto bg-white border-gray-200 flex gap-2 ${groupBy !== "none" ? "bg-primary text-white" : "bg-white"}`}
                  >
                    <img
                      src="/eva-icons (2)/outline/layers.png"
                      alt="group"
                      className={`w-4 h-4 ${groupBy !== "none" ? "invert" : "opacity-60"}`}
                    />
                    <SelectValue
                      placeholder={t("dashboard.group_by", "Group by")}
                    />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="none">
                      {t("dashboard.group.none", "No Grouping")}
                    </SelectItem>
                    <SelectItem value="status">
                      {t("dashboard.group.status", "Status")}
                    </SelectItem>
                    <SelectItem value="date">
                      {t("dashboard.group.date", "Month Created")}
                    </SelectItem>
                    <SelectItem value="location">
                      {t("dashboard.group.location", "Location")}
                    </SelectItem>
                    <SelectItem value="customer">
                      {t("dashboard.group.customer", "Customer")}
                    </SelectItem>
                    {currentUser?.role === "admin" && (
                      <>
                        <SelectItem value="branch">
                          {t("dashboard.group.branch", "Branch")}
                        </SelectItem>
                        <SelectItem value="employee">
                          {t("dashboard.group.employee", "Employee")}
                        </SelectItem>
                      </>
                    )}
                  </SelectContent>
              ...
              {currentView === "trash" && filteredProjects.length > 0 && (
                ...
                >
                  <img
                    src="/eva-icons (2)/outline/trash.png"
                    alt="empty trash"
                    className="w-5 h-5 ltr:mr-2 rtl:ml-2 group-hover:invert"
                  />
      ...
      <Dialog
        ...
        onOpenChange={(isOpen) => !isOpen && closeProjectModal()}
      >
      ...
      <Dialog open={isCreateModalOpen} onOpenChange={setIsCreateModalOpen}>
        <CreateProjectModal
          ...
          initialData={quickCalcData}
        />
      </Dialog>
      <Dialog open={isQuickCalcOpen} onOpenChange={setIsQuickCalcOpen}>
        <QuickCalculateModal
          onConvert={handleConvertQuickCalcToProject}
          onOpenChange={setIsQuickCalcOpen}
        />
      </Dialog>
      <Dialog
      ...
      <AlertDialog
        open={!!projectToDelete || isConfirmingEmptyTrash}
          ...
          <AlertDialogHeader>
            <AlertDialogTitle>
              {isConfirmingEmptyTrash
                ? t(
                    "dashboard.confirm_empty_trash.title",
                    "Are you sure you want to empty the trash?",
                  )
                : t(
                    "dashboard.confirm_delete.title",
            ...
            <AlertDialogDescription>
              {isConfirmingEmptyTrash
                ? t(
                    "dashboard.confirm_empty_trash.description",
                    "All projects in the trash will be permanently deleted. This action cannot be undone.",
                  )
                : t(
                    "dashboard.confirm_delete.description",
