    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent className="w-[--radix-popover-trigger-width] p-0 bg-white rounded-lg">
        <Command>
          ...
          <CommandList>
            <CommandEmpty>
              {t("registration.search.no_results", "No results found.")}
            </CommandEmpty>
            <CommandGroup>
              {items.map((item: any) => (
                <CommandItem
                  ...
                  onSelect={(currentValue) => {
                    ...
                    const original =
                      ...
                      ) || items.find((i: any) => i.label === currentValue);
...
export function SettingsModal({ passwordChange }: SettingsModalProps) {
  ...
  const { setCurrentAuthentication, logout, changePassword } =
  ...
  const isEnterprise = currentUser
    ? currentUser.account_type !== "standard"
    : false;
  ...
  const [emailValidationError, setEmailValidationError] = useState<
    string | null
  >(null); // New state for email validation
  ...
  const [confirmPasswordError, setConfirmPasswordError] = useState<
    string | null
  >(null); // New state for confirm password validation

  // Deactivation confirm dialog
  const [isDeactivateOpen, setIsDeactivateOpen] = useState(false);
  const [deactivatePassword, setDeactivatePassword] = useState("");
  const [isDeactivatePasswordVerified, setIsDeactivatePasswordVerified] =
    useState(false);
  const [isVerifyingDeactivatePassword, setIsVerifyingDeactivatePassword] =
    useState(false);
  ...
  const uniqueStates = useMemo(
    () =>
      Array.from(new Set(geoDataParsed.map((item) => item.state))).map(
        (stateName) => {
  ...
  useEffect(() => {
    ...
    fetchOrganizations();
  }, [lastSyncTime, fetchOrganizations]);
  ...
  const handleSavePersonalProfile = async () => {
    ...
    if (!navigator.onLine) {
      toast.error(
        t(
          "settings.internet_required_save",
          "Internet connection required to save changes",
        ),
      );
      return;
    }
    ...
    if (personalDraft.username.trim() === "") {
      setUsernameError(
        t("settings.username_required", "Username is required."),
      );
      return;
    } else {
      setUsernameError(null);
  ...
  useEffect(() => {
    ...
    if (newPassword.length > 0 && newPassword.length < PASSWORD_MIN_LENGTH) {
      ...
      setNewPasswordError(
        t(
          "registration.password_hint_number",
    ...
    if (confirmPassword.length > 0 && newPassword !== confirmPassword) {
      setConfirmPasswordError(
        t("registration.passwords_mismatch", "Passwords do not match."),
      );
    }
  ...
  const handlePasswordChange = async () => {
    ...
    if (
      ...
      toast.error(
        t(
          "settings.password_validation_error",
    ...
    try {
      ...
      const errorMessage = e?.message?.includes("is incorrect")
        ? t(
            "settings.incorrect_password",
            "The current password you entered is incorrect",
          )
        : e?.message ||
          t("settings.update_failed", "Failed to update settings");
  ...
  useEffect(() => {
    ...
    if (!EMAIL_REGEX.test(email)) {
      setEmailValidationError(
        t("registration.invalid_email", "Email is invalid"),
      );
      return;
    }
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        if (!isUnique) {
          setEmailValidationError(
            t("registration.email_taken", "Email is already in use"),
          );
        }
      } catch (error) {
        if (cancelled) return;
  ...
  const handleUpdateEmail = async () => {
    ...
    if (!navigator.onLine) {
      toast.error(
        t(
          "team.internet_required",
          "Active internet connection required for this action",
        ),
      );
      return;
    }
    if (emailValidationError || emailAvailable === false || !newEmail.trim()) {
      toast.error(
        t(
          "settings.email_validation_error",
          "Please provide a valid and available email.",
        ),
      );
      return;
    }
  ...
  const handleDeactivateAccount = async () => {
    ...
    try {
      ...
      toast.error(
        t("settings.deactivate_failed", "Failed to deactivate account"),
      );
    }
  };

  const handleVerifyDeactivatePassword = async (): Promise<
    boolean | undefined
  > => {
  ...
  const formatRelativeTime = (iso: string | null) => {
    ...
    if (minutes < 60)
      return t("settings.minutes_ago", "{{count}} mins ago", {
        count: minutes,
      });
    const hours = Math.floor(minutes / 60);
    if (hours < 24)
      return t("settings.hours_ago", "{{count}} hrs ago", { count: hours });
  ...
  return (
    ...
    >
      ...
      <Tabs
        ...
        className="flex flex-1 h-full pb-2 overflow-y-auto overscroll-none"
        orientation="vertical"
        dir={i18n.dir()}
      >
        <TabsList
          className="flex flex-col w-64 bg-gray-100 p-4 gap-2 h-full border-[1px] border-primary-gray justify-start rounded-xl m-2"
          dir={i18n.dir()}
        >
          <TabsTrigger
            value="profile"
            className="w-full justify-start gap-3 px-4 py-3 rounded-xl data-[state=active]:bg-primary data-[state=active]:text-white data-[state=active]:shadow-sm transition-all font-bold"
          >
          ...
          <div className="mt-auto pt-4 w-full">
            <Button
              ...
              className="w-full justify-start px-4 py-3 rounded-lg bg-white text-red-700 hover:text-white hover:bg-red-500 font-bold gap-2 transition-colors"
            >
              <LogOut
                className={`w-4 h-4 ${i18n.dir() === "rtl" && "rotate-180"}`}
              />
            ...
            <div className="p-8 space-y-8">
              <TabsContent value="profile" className="m-0 space-y-6">
                <div className="flex items-center gap-6 pb-4">
                  ...
                  <div>
                    <h3 className="text-xl font-black text-neutral/80">
                    ...
                    <p className="text-sm font-bold text-neutral/40 ">
                      ...
                      {isEnterprise
                        ? ` | ${t(`team.role.${currentUser?.role}`, "Role")}`
                        : ""}
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 gap-6">
                                ...
                                <Button
                                  ...
                                  onClick={() => {
                                    if (!navigator.onLine) {
                                      toast.error(
                                        t(
                                          "settings.internet_required_edit",
                                          "Internet connection required to edit settings",
                                        ),
                                      );
                                      return;
                                    }
                    ...
                    <div className="mt-4 flex flex-col md:grid-cols-2 gap-6">
                      <div className="space-y-2">
                        ...
                        {isEditingPersonal ? (
                          ...
                          <p className="h-12 flex items-center font-bold text-neutral/80">
                            {currentUser?.username || "—"}
                          </p>
                        )}
                      </div>
                      ...
                      <div className="flex flex-row gap-6">
                        <div className="space-y-2 w-1/2">
                          ...
                          {isEditingPersonal ? (
                            ...
                            <p className="h-12 flex items-center font-bold text-neutral/80">
                              {currentUser?.location?.split(", ")[1] || "—"}
                            </p>
                          )}
                          {isEmployee && isEditingOrganization && (
                            <p className="text-[10px] text-semantic-warning font-bold ">
                              {t(
                                "settings.employee_location_locked",
                                "Employees cannot edit location",
                              )}
                            </p>
                          )}
                        </div>
                        ...
                        <div className="space-y-2 w-1/2">
                          ...
                          {isEditingPersonal ? (
                            <SearchableSelect
                              ...
                              placeholder={t(
                                "registration.select_city",
                                "Select city",
                              )}
                              disabled={
                                isEmployee || !personalDraft.locationState
                              }
                            />
                          ) : (
                            <p className="h-12 flex items-center font-bold text-neutral/80">
                  ...
                  <div className="relative border border-gray-100 bg-white rounded-2xl p-6">
                    ...
                    <div className="mt-4">
                      <div className="space-y-2">
                        <Label className="text-xs font-bold text-neutral/40 ">
                          {t("registration.business_name", "Business Name")}
                        </Label>
                        {isEditingOrganization ? (
                          <Input
                            value={organizationDraft.business_name}
                            onChange={(e) =>
                              setOrganizationDraft((p) => ({
                                ...p,
                          ...
                          <p className="h-12 w-fit flex items-center font-bold text-neutral/80">
                            {isEnterprise
                        ...
                        {isEnterprise &&
                          ...
                          isEditingOrganization && (
                            <p className="text-[10px] text-semantic-warning font-bold ">
                              {t(
                                "settings.employee_org_locked",
                                "Only admins can edit organization profile",
                              )}
                            </p>
                          )}
                      </div>
              ...
              <TabsContent value="security" className="m-0 space-y-6">
                <div className="space-y-4">
                  ...
                  <div className="space-y-4 max-w-sm">
                    ...
                    >
                      ...
                      {t("auth.change_password_now", "Update Password")}
                    </Button>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <h4 className="text-sm font-black text-neutral/80  text-primary">
                    {t("settings.change_email", "Change Email")}
                  </h4>
                  <div className="space-y-4 max-w-sm">
                    <div className="space-y-2">
                      <Label className="text-xs font-bold text-neutral/40 ">
                ...
                {!isEmployee && (
                  <>
                    <Separator />
                    <div className="space-y-4">
                      <h4 className="text-sm font-black text-red-500  text-primary">
                        ...
                        <div className="flex items-start justify-between gap-6">
                          <div className="space-y-0.5">
                            <Label className="font-bold text-red-600">
                              {t("settings.deactivate", "Deactivate Account")}
                            </Label>
                          </div>
                        </div>
                            ...
                            >
                              {isDeactivatePasswordVerified ? (
                                <div className="flex flex-row gap-2 h-12 items-center">
                                  ...
                                  {t(
                                    "settings.deactivate_button",
                                    "Deactivate",
                                  )}
                                </div>
                              ) : (
                                <div>{t("common.verify", "Verify")}</div>
                              )}
                            </Button>
                          </div>
                        </div>
                      ...
                      >
                        <AlertDialogContent className="bg-white">
                          <AlertDialogHeader>
                            <AlertDialogTitle>
                              {t(
                                "settings.confirm_deactivate_title",
                                "Deactivate Account?",
                              )}
                            </AlertDialogTitle>
                            <AlertDialogDescription>
                              {t(
                                "settings.confirm_deactivate_desc",
                                "This will disable access and cascade deactivation to all related data. This action is non-recoverable.",
                              )}
                            </AlertDialogDescription>
                          </AlertDialogHeader>
              ...
              <TabsContent value="system" className="m-0 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <Card className="border-[1px] border-primary-gray bg-gray-50 p-6 flex flex-col gap-2 rounded-2xl">
                    <p className="text-center text-primary text-xs font-bold text-neutral/40 ">
                  ...
                  <Card className="border-[1px] border-primary-gray bg-gray-50 p-6 flex flex-col gap-2 rounded-2xl">
                    <p className="text-center text-primary text-xs font-bold text-neutral/40 ">
                      {t("settings.db_size", "Local DB Size")}
                    </p>
                    <p className="text-center text-2xl font-black text-neutral/80">
                      {isSystemInfoLoading
                        ? t("common.loading", "Loading...")
                        : formatBytes(systemInfo?.local_db_size_bytes ?? 0)}
                    </p>
                  </Card>
                  <Card className="border-[1px] border-primary-gray bg-gray-50 p-6 flex flex-col gap-2 rounded-2xl">
                    <p className="text-center text-primary text-xs font-bold text-neutral/40 ">
                      {t("settings.last_sync", "Last Sync")}
                    </p>
                ...
                <div className="space-y-2 w-1/2">
                  <Label className="text-xs text-primary font-bold ">
                    {t("registration.language.placeholder", "Language")}
                  </Label>
                  <Select
                  ...
                  >
                    <SelectTrigger className="bg-white border-[1px] border-primary-gray rounded-xl h-12 font-medium">
                      <SelectValue
                        placeholder={t(
                          "settings.select_language_placeholder",
                          "Select language",
                        )}
                      />
                    </SelectTrigger>
                  ...
                  <div className="p-4 border border-primary-gray bg-white rounded-2xl flex items-center justify-between group hover:border-primary/30 transition-all">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="text-sm font-bold text-neutral/80">
                          {t("tc.terms_title", "Terms and Conditions")}
                        </p>
                    ...
                    <Dialog>
                      <DialogTrigger asChild>
                        <Button
                          variant="outline"
                          className="rounded-xl font-bold h-10 border-primary-gray hover:bg-primary hover:text-white transition-all"
                        >
                          {t("tc.view", "View")}
                        </Button>
                      </DialogTrigger>
                      <DialogContent className="max-w-2xl max-h-[85vh] bg-white border-none rounded-3xl shadow-2xl flex flex-col pb-6 overflow-hidden">
                        <DialogHeader className="p-6 pb-2 bg-gray-50/50">
                          <DialogTitle className="text-2xl font-black text-neutral/80">
                            {t("tc.terms_title", "Terms and Conditions")}
                          </DialogTitle>
                        </DialogHeader>
