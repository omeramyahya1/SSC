...
export function ProjectCard({
  ...
  const { archiveProject, softDeleteProject, recoverProject } =
    useProjectStore();
  ...
  const branch = branches.find((b) => b.uuid === project.branch_uuid);
  const employee = users.find((u) => u.uuid === project.user_uuid);
    ...
    >
        ...
        <div className="flex-grow overflow-hidden">
          ...
          <div className="flex flex-col gap-1 pt-1">
            <div className="flex items-center gap-1 text-sm text-muted-foreground truncate">
              <img
                src="/eva-icons (2)/outline/pin.png"
                alt="location"
                className="w-4 h-4 opacity-70 flex-shrink-0"
              />
              <p className="truncate" title={displayLocation}>
                {displayLocation}
              </p>
            </div>
            <div className="flex items-center gap-1 text-xs text-muted-foreground truncate">
              <img
                src="/eva-icons (2)/outline/calendar.png"
                alt="date"
          ...
          {viewMode === "trash" ? (
            <div className="flex items-center">
              <Button
                variant="ghost"
                size="icon"
                className="h-8 w-8 group text-green-600 hover:text-green-700 hover:bg-green-500"
                onClick={handleRecover}
                title={t("dashboard.recover", "Recover")}
              >
                <img
                  src="/eva-icons (2)/outline/undo.png"
                  alt="recover"
              ...
              <Button
                variant="ghost"
                size="icon"
                className="h-8 w-8 group text-red-600 hover:bg-red-500"
                onClick={(e) => {
                  e.stopPropagation();
                  onPermanentDelete(project);
                }}
                title={t("dashboard.permanent_delete", "Delete Permanently")}
              >
                <img
                  src="/eva-icons (2)/outline/delete-bin-2.svg"
            ...
            <DropdownMenu dir={i18n.dir()}>
              <DropdownMenuTrigger asChild>
                ...
                >
                  <img
                    src="/eva-icons (2)/outline/more-vertical.png"
                    alt="options"
                    className="w-5 h-5"
                  />
                </Button>
                ...
                {project.status !== "archived" && (
                  ...
                  >
                    <img
                      ...
                      alt="archive"
                      className="w-4 h-4 ltr:mr-2 rtl:ml-2 opacity-70"
                    />
                    {t("dashboard.archive", "Archive")}
                  </DropdownMenuItem>
                )}
                <DropdownMenuItem
                  className="cursor-pointer rounded-lg group text-red-700 border-red-200 hover:text-white  hover:bg-red-500"
                  onClick={handleDelete}
                >
                  <img
                    src="/eva-icons (2)/outline/trash-2.png"
                    alt="delete"
                    className="w-4 h-4 ltr:mr-2 rtl:ml-2 opacity-70 group-hover:invert"
                  />
      ...
      <CardContent className="p-4 pt-0 mt-auto">
              ...
              <>
                <Badge variant="secondary" className="font-medium">
                  <img
                    src="/eva-icons (2)/outline/pin.png"
                    alt="branch"
                    className="w-3 h-3 ltr:mr-1 rtl:ml-1 opacity-70"
                  />
                  {branch ? branch.name : t("dashboard.unknown_branch", "Unknown Branch")}
                </Badge>
                <Badge variant="outline" className="font-medium">
                  <img
                    src="/eva-icons (2)/outline/person.png"
                    alt="employee"
                    className="w-3 h-3 ltr:mr-1 rtl:ml-1 opacity-70"
                  />
                  {employee ? employee.username : t("dashboard.unknown_employee", "Unknown Employee")}
                </Badge>
              </>
            )}
          </div>
        ...
        {viewMode === "trash" && (
          <Badge
            variant="outline"
            className="font-semibold bg-red-50 text-red-700 border-red-200"
          >
