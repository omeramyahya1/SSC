...
export function InvoicesList({ filterParams }: InvoicesListProps) {
  ...
  const { invoices, fetchInvoices, deleteInvoice, isLoading } =
    useInvoiceStore();
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      toast.success(
        t("invoicing.delete_success", "Invoice deleted successfully."),
      );
      fetchInvoices(filterParams);
    } catch (error: any) {
      toast.error(
        error.message ||
          t("invoicing.delete_error", "Failed to delete invoice."),
      );
    } finally {
  ...
  return (
    <div className="space-y-4" dir={i18n.dir()}>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-4 flex-grow max-w-4xl">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <SelectTrigger
                className={`w-fit h-10 rounded-xl ${statusFilter === "all" ? "bg-white" : "bg-primary text-white"}`}
              >
                <SelectValue
                  placeholder={t("finances.filter_status", "Status")}
                />
              </SelectTrigger>
              <SelectContent className="bg-white">
                <SelectItem value="all">
                  {t("finances.all", "All (status)")}
                </SelectItem>
                <SelectItem value="draft">
                  {t("finances.draft", "Draft")}
                </SelectItem>
            ...
            <Select value={sortBy} onValueChange={setSortBy} dir={i18n.dir()}>
              <SelectTrigger
                className={`w-fit h-10 rounded-xl ${sortBy === "date-desc" ? "bg-white" : "bg-primary text-white"}`}
              >
              ...
              <SelectContent className="bg-white">
                <SelectItem value="date-desc">
                  {t("common.sort.date_new", "Newest First")}
                </SelectItem>
                <SelectItem value="date-asc">
                  {t("common.sort.date_old", "Oldest First")}
                </SelectItem>
                <SelectItem value="amount-desc">
                  {t("common.sort.amount_high", "Amount: High-Low")}
                </SelectItem>
                <SelectItem value="amount-asc">
                  {t("common.sort.amount_low", "Amount: Low-High")}
                </SelectItem>
                <SelectItem value="remainder-desc">
                  {t("common.sort.remainder_high", "Remainder: High-Low")}
                </SelectItem>
              </SelectContent>
            </Select>
          </div>
          ...
          <TableHeader className="bg-gray-50/50">
            <TableRow>
              ...
              <TableHead className="text-start font-bold">
                {t("invoicing.customer", "Customer")}
              </TableHead>
              <TableHead className="text-start font-bold">
                {t("invoicing.due_date", "Due Date")}
              </TableHead>
              <TableHead className="text-start font-bold">
                {t("invoicing.amount", "Amount")}
              </TableHead>
              <TableHead className="text-start font-bold">
                {t("invoicing.paid", "Paid")}
              </TableHead>
              <TableHead className="text-start font-bold">
                {t("invoicing.remainder", "Remainder")}
              </TableHead>
              <TableHead className="text-center font-bold">
                {t("invoicing.status", "Status")}
              </TableHead>
              <TableHead className="text-end font-bold">
                {t("invoicing.actions", "Actions")}
              </TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {filteredAndSortedInvoices.map((invoice) => (
              <TableRow
                key={invoice.uuid}
                className="text-start hover:bg-gray-50/50 transition-colors"
              >
                ...
                <TableCell className="font-black text-gray-900">
                  {invoice.issued_at ? formatCurrency(invoice.amount) : "-"}
                </TableCell>
                ...
                <TableCell
                  className={`font-black ${(invoice as any).remainder > 0 ? `text-red-500` : `text-green-600`}`}
                >
                  {invoice.issued_at
                ...
                <TableCell className="text-end">
                  <DropdownMenu dir={i18n.dir()}>
                    ...
                    <DropdownMenuContent
                      align="end"
                      className="w-48 bg-white p-2 rounded-xl"
                    >
                      {invoice.status !== "paid" &&
                        invoice.issued_at &&
                        (invoice.access
                          ? invoice.access.mode === "full" &&
                            invoice.access.can_add_payment !== false
                          : true) && (
                          <DropdownMenuItem
                            onClick={() => {
                      ...
                      {(invoice.access
                        ? invoice.access.mode === "full" &&
                          invoice.access.can_delete !== false
                        : true) && (
                        <DropdownMenuItem
                          onClick={() => setInvoiceToDelete(invoice)}
                          className="cursor-pointer rounded-lg text-red-700 hover:text-white hover:bg-red-500 font-bold gap-2"
                        >
      ...
      <AlertDialog
        ...
        onOpenChange={(isOpen) => !isOpen && setInvoiceToDelete(null)}
      >
        <AlertDialogContent className="bg-white border-2">
          <AlertDialogHeader>
            <AlertDialogTitle>
              {t(
                "invoicing.confirm_delete.title",
                "Are you sure you want to delete this invoice?",
              )}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {t(
                "invoicing.confirm_delete.description",
                "This action will reverse inventory deductions for issued invoices. This cannot be undone.",
              )}
            </AlertDialogDescription>
            <div className="mt-3 flex items-center gap-2">
              ...
              />
              <label
                htmlFor="cascade-payments"
                className="text-sm text-muted-foreground"
              >
                {t(
                  "invoicing.confirm_delete.cascade_payments",
