...
export function IndependentInvoiceEditor({
  invoiceUuid,
  user,
  onBack,
}: IndependentInvoiceEditorProps) {
  ...
  const projectLocation =
    (currentInvoice?.invoice_details as any)?.project_location || "";
  ...
  const handleAddManualItem = () => {
    setManualItems((prev) => [
      ...(prev || []),
      { id: crypto.randomUUID(), name: "", quantity: 1, price: 1 },
    ]);
  };

  const updateManualItem = (
    id: string,
    updates: Partial<Pick<ManualItem, "name" | "quantity" | "price">>,
  ) => {
    setManualItems((prev) =>
      (prev || []).map((item) =>
  ...
  const updateInventoryItem = (
    id: string,
    updates: Partial<Pick<InventoryLineItem, "quantity" | "unit_price">>,
  ) => {
    setInventoryItems((prev) =>
      (prev || []).map((item) =>
        item.id === id ? { ...item, ...updates } : item,
  ...
  const handleSaveInvoice = async () => {
    ...
    const details: InvoiceDetails = {
      ...
      project_location: (currentInvoice.invoice_details as any)
        ?.project_location,
    };
  ...
  const handleExport = async (type: "pdf" | "excel" | "csv") => {
    ...
    try {
      ...
      const response = await api.get(
        `/export/${type === "excel" ? "excel" : type}/invoice/${currentInvoice.uuid}`,
        {
          params:
          ...
          <div className="flex flex-col md:flex-row justify-between gap-8 pb-8 border-b">
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2 text-muted-foreground">
                ...
                <span className="text-sm font-bold">
                  {t("invoicing.email", "Email")}: {customer?.email || "N/A"}
                </span>
              </div>
              <div className="flex items-center gap-2 text-muted-foreground">
                <Phone className="h-4 w-4 shrink-0" />
                <span className="text-sm font-bold">
                  {t("invoicing.phone", "Phone No")}:{" "}
                    ...
                    <PopoverTrigger asChild>
                      ...
                      >
                        ...
                        {dueDate ? (
                          ...
                          <span>{t("invoicing.pick_date", "Pick a date")}</span>
                        )}
                      </Button>
                    </PopoverTrigger>
                    <PopoverContent
                          ...
                          <Input
                            ...
                            onChange={(e) => {
                              ...
                              updateInventoryItem(item.id, {
                                unit_price: Math.max(1, isFinite(p) ? p : 1),
                              });
                            }}
                            min={1}
