...
import { Circle, MinusCircle, Edit } from "lucide-react";

interface EmployeesTabProps {
  employees: User[];
  maxEmployees: number;
  onAddEmployee: () => void;
  onEmployeeEdit: (employee: User) => void;
          ...
          <div className="flex flex-row gap-4">
            <div className="font-bold">
              {i18n.dir() === "ltr" ? (
                <>
                  <span
                    className={`text-4xl ${isLimitReached ? "text-red-500 font-bold" : "text-primary"}`}
                  >
                    {employees.length}
                  </span>
      ...
      {filteredEmployees.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-64 bg-white rounded-xl border border-dashed border-gray-300 p-8 text-center">
          <h3 className="text-lg font-bold text-gray-400">
            {searchQuery
              ? t("common.no_results", "No results match your search")
              : t("team.no_employees", "No employees found")}
          </h3>
        </div>
      ) : (
              ...
              <CardHeader className="p-4 flex flex-row items-start justify-between space-y-0">
                <div className="flex flex-col gap-1">
                  ...
                  <div className="flex items-center  text-sm text-muted-foreground">
                    <Circle
                      className={`h-4 w-4 text-white rounded-full ${emp.status === "active" ? "bg-semantic-success" : emp.status === "trial" ? "bg-semantic-info" : emp.status === "grace" ? "bg-semantic-warning" : "bg-semantic-error"}`}
                    />
                    <Badge
                      variant="secondary"
                  ...
                  <DropdownMenu dir={i18n.dir()}>
                    ...
                    <DropdownMenuContent align="end" className="bg-white">
                      ...
                      >
                        ...
                        {t("team.change_branch", "Change Branch")}
                      </DropdownMenuItem>
                      <DropdownMenuItem
              ...
              <CardContent className="p-4 pt-0 space-y-3">
                <div className="text-sm space-y-2">
                  <div className="flex items-center gap-2 text-muted-foreground">
                    <img
                      src="/eva-icons (2)/outline/email.png"
                      alt="email"
