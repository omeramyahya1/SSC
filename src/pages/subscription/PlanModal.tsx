...
export function PlanModal({ isOpen, onOpenChange }: PlanModalProps) {
  ...
  const { subscriptions, currentSubscription, fetchSubscriptions } =
  ...
  const [view, setView] = useState<
    "status" | "upgrade" | "payment" | "activate"
  >("status");
  ...
  const [tier1Duration, setTier1Duration] = useState<
    "Monthly" | "Annual" | "Lifetime"
  >("Monthly");

  // Payment stateonOpenChange
  const [paymentMethod, setPaymentMethod] = useState<string | null>(null);
  const [confirmedTransfer, setConfirmedTransfer] = useState<string | null>(
    null,
  );
  ...
  useEffect(() => {
    const fetchDistributorInfo = async () => {
      if (currentUser?.distributor_id && !discountApplied) {
        try {
          const response = await api.post("/users/distributor-info", {
  ...
  useEffect(() => {
    ...
    isOpen,
    hasInitializedEmployees,
    isUsersLoading,
    users.length,
    currentUser?.uuid,
    currentUser?.organization_uuid,
    currentOrganization?.emp_count,
  ]);
  ...
  const hasPendingPayment = useMemo(() => {
    ...
    return subscriptionPayments.some(
      (p) =>
        p.status === "under_processing" &&
        p.subscription_uuid === latestSubscription.uuid,
    );
  ...
  const calculatedPrice = useMemo(() => {
    ...
    if (accountType === "standard") {
      const planInfo = pricingData.find(
        (p) =>
          p.plan_type.toLowerCase() === "standard" &&
          p.billing_cycle.toLowerCase() === selectedPlan.toLowerCase(),
      );
      ...
      const planInfo = pricingData.find(
        (p) =>
          ...
          p.billing_cycle.toLowerCase() === tier1Duration.toLowerCase(),
      );

      if (planInfo) {
        const basePrice = planInfo.base_price;
        const extraEmployeeCost =
          employees > 1
            ? (employees - 1) * planInfo.price_per_extra_employee
            : 0;
        const totalBeforeDiscount = basePrice + extraEmployeeCost;

        const discountInfo = pricingData.find(
          (p) =>
            p.plan_type.toLowerCase() === "enterprise" &&
            p.min_employees &&
            p.max_employees &&
            employees >= p.min_employees &&
            employees <= p.max_employees,
        );
  ...
  const handleApplyReferral = async () => {
    ...
    try {
      const response = await api.post("/users/check-referral", {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      if (!file.type.startsWith("image/")) {
        toast.error(
          t(
            "subscription.invalid_receipt_type",
      ...
      if (file.size > MAX_RECEIPT_SIZE_BYTES) {
        toast.error(
          t("subscription.receipt_too_large", "Receipt image is too large."),
        );
        return;
      }
  ...
  const handleSubmitPayment = async () => {
    if (!isOnline) {
      toast.error(
        t("common.internet_required", "Internet connection required"),
      );
      return;
    }
    ...
    try {
      ...
      const subscriptionCreationData = {
        ...
        plan_type:
          selectedPlan === "Tier1" ? "enterprise" : selectedPlan!.toLowerCase(),
        billing_cycle: tier1Duration,
        employees: employees,
      };
      const subscriptionResponse = await api.post(
        "/subscriptions/create-and-sync",
        subscriptionCreationData,
      );
      const newSubscriptionUuid =
        subscriptionResponse.data.new_subscription_uuid;

      if (!newSubscriptionUuid) {
        throw new Error(
          t(
            "subscription.failed_create_sub",
      ...
      try {
        ...
        if (!response)
          throw new Error(
            t("subscription.payment_failed", "Failed to submit payment"),
          );
      } catch (paymentError) {
        try {
          ...
          console.error(
            `Failed to cancel orphaned subscription ${newSubscriptionUuid}:`,
            cancelError,
          );
        }
        throw paymentError;
      ...
      toast.success(
        t("subscription.payment_submitted", "Payment submitted for review"),
      );
      await triggerSync();
      onOpenChange(false);
    } catch (error: any) {
  ...
  const handleActivateLicense = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        toast.success(
          t(
            "subscription.activation_success",
      ...
      if (error.response?.status === 401) {
        toast.error(
          t("subscription.activation_failed", "Invalid license code"),
        );
      ...
      return (
        ...
        >
          ...
          <p className="text-neutral/70 font-bold">
            {t(
              "subscription.tampered_desc",
              "Unusual activity detected on your account. Access to online features has been restricted for security reasons.",
            )}
          </p>
          <Button
        ...
        >
          ...
          <div>
            <h3 className="text-xl font-bold mb-2">
              {t("subscription.org_subscribed", "Organization Subscription")}
            </h3>
            <p className="text-neutral/60">
              {t(
                "subscription.org_member_desc",
                "You are part of the {{org}} organization.",
          ...
          {(isGrace || isExpired) && (
            <Alert variant="destructive" className="bg-red-50 border-red-200">
              ...
              <AlertTitle>
                {t("subscription.action_required", "Action Required")}
              </AlertTitle>
              <AlertDescription>
                {t(
                  "subscription.contact_admin_renew",
                  "Your organization's plan has expired. Please contact your administrator to renew.",
                )}
              </AlertDescription>
            </Alert>
          )}
    ...
    return (
      <div className="space-y-6 p-4" dir={i18n.dir()}>
        <div className="flex flex-col items-center gap-2 pb-4 border-b">
          ...
          <h2 className="text-3xl font-black text-primary capitalize">
            {t(
              `registration.plans.${currentSubscription?.type}`,
          ...
          >
            {isGrace
              ? t("dashboard.grace", "Grace Period")
              : isExpired
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="p-4 bg-gray-50 rounded-xl border border-gray-100">
            <span className="text-[10px] font-bold text-neutral/40 block mb-1">
              {t("subscription.expires_on", "Expires On")}
            </span>
          ...
          <div className="p-4 bg-gray-50 rounded-xl border border-gray-100">
            <span className="text-[10px] font-bold text-neutral/40 block mb-1">
              {t("subscription.account_type", "Account Type")}
            </span>
            <span className="text-sm text-primary font-bold capitalize">
              {t(
                `registration.type.${currentUser?.account_type}`,
                `${currentUser?.account_type}`,
              )}
            </span>
          </div>
        ...
        {currentSubscription?.license_code && (
          <div className="p-4 bg-primary/5 rounded-xl border border-primary/10 text-center">
            <span className="text-[10px] font-bold text-primary/60 uppercase block mb-2">
              {t("subscription.your_license", "Your License Code")}
            </span>
            <code className="text-lg font-mono font-bold select-all tracking-wider">
              {currentSubscription.license_code}
            </code>
          </div>
        )}
        ...
        <div className="flex flex-col items-center gap-2 pt-4">
          {(isGrace || isExpired) && isAdmin && (
            <Button
              className="h-12 text-lg font-bold text-white"
              onClick={() =>
                setView(hasPendingPayment ? "activate" : "upgrade")
              }
            >
          ...
          <Button
            variant="link"
            className="text-s opacity-80 w-fit"
            onClick={triggerSync}
            disabled={isSyncing}
          >
  ...
  const renderUpgrade = () => {
    ...
    if (isActive && currentType) {
      plans = plans.filter(
        (p) => p.toLowerCase() !== currentType.toLowerCase(),
      );
    }
      ...
      <div className="space-y-6" dir={i18n.dir()}>
        {!hasPendingPayment && (
          <div className="flex flex-col text-center items-center gap-4 mb-4">
            <label className="font-bold w-full">
              {t("subscription.choose_plan", "Choose your new plan")}
            </label>
          </div>
        )}
          ...
          <div className="grid grid-cols-1 gap-4">
              ...
              plans.map((plan) => (
                ...
                >
                  <CardHeader className="p-4 flex flex-row items-center justify-between space-y-0">
                    <div>
                      <CardTitle className="text-lg text-start font-black">
                        {t(`registration.plans.${plan.toLowerCase()}`, plan)}
                      </CardTitle>
                      <CardDescription className="text-xs">
                        {isEnterprise
                          ? t(
                              "registration.plans.tier1_desc",
                    ...
                    <CardContent className="p-4 pt-0 space-y-4">
                      <div className="space-y-2">
                        <Label className="text-xs font-bold">
                          {t(
                            "registration.allowed_employees",
                            "Allowed Employees",
                          )}
                          : {employees}
                        </Label>
                        ...
                        {employees === minAllowed && (
                          <p className="text-[10px] text-semantic-error italic ps-1">
                            {t(
                              "subscription.min_employees_hint",
                              "Minimum employees reached. Deactivate employees in the Team section to reduce this count.",
                            )}
                          </p>
                        )}
                      </div>
                      <div className="space-y-2">
                        <Label className="text-xs font-bold">
                          {t("registration.duration", "Duration")}
                        </Label>
                        <Select
                          value={tier1Duration}
                          onValueChange={(v: any) => setTier1Duration(v)}
                          dir={i18n.dir()}
                        >
                          ...
                          <SelectContent className="bg-white">
                            <SelectItem value="Monthly">
                              {t("registration.plans.monthly", "Monthly")}
                            </SelectItem>
                            <SelectItem value="Annual">
                              {t("registration.plans.annual", "Annual")}
                            </SelectItem>
                            <SelectItem value="Lifetime">
                              {t("registration.plans.lifetime", "Lifetime")}
                            </SelectItem>
                          </SelectContent>
                        </Select>
                      </div>
            ...
            {!hasPendingPayment && (
              <span className="text-center font-semibold">
                {t("subscription.current_plan", "Your current plan is:")}{" "}
                {t(`registration.plans.${currentType}`, `${currentType}`)}
              </span>
            )}
          </div>
        )}
        ...
        {hasPendingPayment && (
          <Alert className="bg-blue-50 border-blue-200">
            ...
            <AlertTitle className="text-blue-800 font-bold">
              {t("subscription.payment_pending", "Payment Under Review")}
            </AlertTitle>
            <AlertDescription className="text-blue-700">
              {t(
                "subscription.pending_desc",
                "You already have a payment pending review. Please wait for the administrator to approve it.",
              )}
            </AlertDescription>
          </Alert>
        )}
        ...
        {!discountApplied && !hasPendingPayment && (
          <div className="flex flex-col gap-2 p-4 bg-gray-50 rounded-2xl border border-dashed border-gray-300">
            <Label className="text-xs font-bold ps-1">
              {t("registration.referral_code", "Referral Code (Optional)")}
            </Label>
            ...
            {referralStatus === "valid" && (
              <p className="text-[10px] text-green-600 font-bold ps-1">
                {t("registration.referral_valid", "Discount Applied!")}
              </p>
            )}
            {referralStatus === "invalid" && (
              <p className="text-[10px] text-red-600 font-bold ps-1">
                {t("registration.referral_invalid", "Invalid referral code")}
              </p>
            )}
          </div>
        )}
        ...
        {discountApplied && !hasPendingPayment && (
          <Alert className="bg-green-50 border-green-200">
            ...
            <AlertTitle className="text-green-800 font-bold">
              {t("subscription.discount_active", "Discount Active")}
            </AlertTitle>
            <AlertDescription className="text-green-700">
              {t(
                "subscription.discount_desc",
                "A {{percent}}% discount has been applied to your total.",
    ...
    return (
      <div className="space-y-6" dir={i18n.dir()}>
        <div className="flex items-center align-middle mb-4">
          ...
          >
            <ArrowLeft
              className={`${i18n.dir() === "ltr" ? "" : "rotate-180"}`}
            />
          </Button>
          <h2 className="text-xl font-bold">
            {t("registration.stage6.title", "Payment Options")}
          </h2>
        </div>

        <div className="text-center p-6 bg-primary/5 rounded-2xl border border-primary/10">
          <span className="text-xs font-bold text-primary/60 uppercase block mb-1">
            {t("registration.amount_to_pay", "Amount to Pay")}
          </span>
        ...
        >
          {["Bankak", "Ocash", "Fawry", "MyCashi", "BNMB"].map((method) => (
            <AccordionItem
              key={method}
              value={method}
              className="border rounded-xl px-4 mb-2"
            >
              <AccordionTrigger
                onClick={() => setConfirmedTransfer(null)}
                className="hover:no-underline py-4"
              >
                <div className="flex items-center gap-4 w-full">
                  <div className="w-10 h-10 rounded-lg overflow-hidden border">
                    <img
                      src={`/bank_icons/${method.toLowerCase()}.jpg`}
                      className="w-full h-full object-cover"
                      alt={method}
                    />
                  </div>
                  <span className="font-bold">
                    {t(`finances.methods.${method}`, `${method}`)}
                  </span>
                </div>
              </AccordionTrigger>
                  ...
                  <div className="flex flex-col items-center gap-6 pt-4 border-t">
                    <div className="text-center space-y-1">
                      <p className="text-xs font-bold text-neutral/40 uppercase">
                        {t("registration.account_name", "Account Name")}
                      </p>
                      <p className="text-lg font-black">
                        {bankDetails[method.toLowerCase()]?.account_name}
                      </p>
                      <p className="text-xs font-bold text-neutral/40 uppercase mt-4">
                        {t("registration.account_number", "Account Number")}
                      </p>
                      <p className="text-2xl font-mono font-black text-primary tracking-widest">
                    ...
                    {bankDetails[method.toLowerCase()]?.qr_code && (
                      <img
                        ...
                        className="w-48 h-48 rounded-xl border border-primary-gray shadow-xl"
                        alt="QR"
                      />
                    )}
                    <div className="flex items-center gap-2 pt-4 w-full justify-center">
                      <Checkbox
                        id={`confirm-${method}`}
                        checked={confirmedTransfer === method}
                        onCheckedChange={(checked) => {
                          setConfirmedTransfer(checked ? method : null);
                          if (checked) {
                            // Clear reference number and receipt if a new method is checked
                            setReferenceNumber("");
                            setReceipt(null);
        ...
        {confirmedTransfer && (
          <div className="space-y-4 animate-in slide-in-from-bottom-4 duration-300">
            ...
            <div className="space-y-1.5">
              <Label className="text-xs font-bold ps-1">
                {t("registration.upload_receipt", "Upload Receipt")}
              </Label>
              <div
                className="border-2 border-dashed rounded-xl p-8 flex flex-col items-center justify-center cursor-pointer bg-gray-50 hover:bg-gray-100 transition-all relative"
                ...
                {receiptPreview ? (
                  ...
                  <div className="flex flex-col items-center gap-2 opacity-40">
                    ...
                    <span className="text-sm font-bold">
                      {t(
                        "registration.click_upload_receipt",
        ...
        >
          {isSubmittingPayment ? (
            ...
            t("registration.finish", "Submit Payment")
          )}
        </Button>
      </div>
    );
      ...
      <div className="space-y-6" dir={i18n.dir()}>
        ...
        <div className="p-8 bg-primary/5 rounded-3xl border border-primary/10 flex flex-col items-center text-center space-y-4">
          {isActivating ? (
            ...
            <div className="bg-semantic-success p-4 rounded-full">
              <Check className="w-12 h-12 text-white" />
            </div>
          ) : (
            <p className="text-sm font-bold text-neutral/60">
              {t(
                "subscription.activate_desc",
                "Enter the license code sent to your email after payment approval.",
              )}
            </p>
          )}
        </div>
        ...
        <div className="space-y-2">
          <Label className="text-xs font-bold uppercase tracking-widest text-neutral/40 ps-1">
            {t("subscription.license_code", "License Code")}
          </Label>
          <Input
            value={licenseCode}
            onChange={(e) => {
            ...
            maxLength={13}
            className={cn(
              "h-14 text-center font-mono text-xl font-black tracking-widest uppercase",
              invalidLicense ? "border-semantic-error" : "",
            )}
          />
        </div>
        ...
        <div className="flex flex-col gap-2 pt-4">
          ...
          >
            {isActivating ? (
              ...
              t("subscription.activate_now", "Activate Now")
            )}
          </Button>
          {!isOnline && (
            <div className="flex items-center justify-center gap-2 text-red-600 text-sm font-bold">
              ...
              <span>
                {t(
                  "subscription.internet_required_activate",
      ...
      <DialogContent className="max-w-md p-0 overflow-hidden bg-white border-none rounded-3xl shadow-2xl">
        <DialogHeader className="p-6 pb-2">
          <DialogTitle className="">
            {t("subscription.modal_title", "Subscription")}
          </DialogTitle>
        </DialogHeader>
