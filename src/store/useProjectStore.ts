...
} from "@/pages/dashboard/CreateProjectModal";
...
export interface ProjectStore {
  ...
  createProject: (
    data: NewProjectData,
    quickCalcData?: QuickCalcConvertedData,
...
export const useProjectStore = create<ProjectStore>((set, get) => ({
  ...
  getQuickCalcProjectId: async () => {
    ...
    try {
      const { data: project } = await api.post<Project>(
        `${resource}/quick-calc-init`,
      );
      set({ _quickCalcProjectId: project.uuid, isLoading: false });
      return project.uuid;
    } catch (e: any) {
      const errorMsg =
        e.message || "Failed to get or create Quick Calc project";
      set({ error: errorMsg, isLoading: false });
      console.error(errorMsg, e);
      throw e;
    }
  ...
  createProject: async (newProjectData, quickCalcData) => {
    ...
    try {
      ...
      const { data: finalProject } = await api.post<Project>(
        `${resource}/create_with_customer`,
        payload,
      );

      set((state) => ({
        projects: state.projects.map((p) =>
          p.project_id === tempId ? finalProject : p,
        ),
      }));
      return finalProject;
    } catch (e: any) {
  ...
  updateProject: async (
    ...
    const updatedProject = {
      ...projectToUpdate,
      project_location:
        data.project_location ?? projectToUpdate.project_location,
      customer: {
        ...
        phone_number:
          data.phone_number ?? projectToUpdate.customer.phone_number,
      },
      is_pending: true,
    };

    set({
      projects: originalProjects.map((p) =>
        p.uuid === projectUuid ? updatedProject : p,
      ),
    });

    try {
      const { data: finalProject } = await api.patch<Project>(
      ...
      set((state) => ({
        projects: state.projects.map((p) =>
          p.uuid === projectUuid ? { ...finalProject, is_pending: false } : p,
        ),
      }));
      return finalProject;
    } catch (e: any) {
  ...
  softDeleteProject: async (projectUuid: string) => {
    ...
    set((state) => ({
      projects: state.projects.map((p) =>
        p.uuid === projectUuid
          ? { ...p, deleted_at: new Date().toISOString(), is_pending: true }
          : p,
      ),
    }));
    ...
    try {
      ...
      set((state) => ({
        projects: state.projects.map((p) =>
          p.uuid === projectUuid
            ? { ...p, deleted_at: result.deleted_at, is_pending: false }
            : p,
        ),
      }));
    } catch (e: any) {
  ...
  recoverProject: async (projectUuid: string) => {
    ...
    set((state) => ({
      projects: state.projects.map((p) =>
        p.uuid === projectUuid
          ? { ...p, deleted_at: null, is_pending: true }
          : p,
      ),
    }));
