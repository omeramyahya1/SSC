...
@payment_bp.route('/<string:item_id>', methods=['PUT'])
def update_payment(item_id):
    with get_db() as db:
        item = get_by_id_or_uuid(db, Payment, Payment.payment_id, Payment.uuid, item_id)
        if not item:
...
@payment_bp.route('/<string:item_id>', methods=['DELETE'])
