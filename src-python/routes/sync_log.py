...
@sync_log_bp.route('/<int:item_id>', methods=['PUT'])
def update_sync_log(item_id):
    with get_db() as db:
        item = db.query(SyncLog).filter(SyncLog.sync_id == item_id).first()
        if not item:
            return jsonify({"error": "Not found"}), 404
            
        try:
            # Validate request data
            validated_data = SyncLogUpdate(**request.json)
        except ValidationError as e:
            return jsonify({"errors": e.errors()}), 400
...
@sync_log_bp.route('/<int:item_id>', methods=['GET'])
def get_sync_log(item_id):
    with get_db() as db:
        item = db.query(SyncLog).filter(SyncLog.sync_id == item_id).first()
        if not item:
            return jsonify({"error": "Not found"}), 404
...
@sync_log_bp.route('/<int:item_id>', methods=['DELETE'])
def delete_sync_log(item_id):
    with get_db() as db:
        item = db.query(SyncLog).filter(SyncLog.sync_id == item_id).first()
        if not item:
            return jsonify({"error": "Not found"}), 404
